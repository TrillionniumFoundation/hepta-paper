...
import {
  REPORT_CONTRACT_DOC_PAGE_BOOTSTRAP_SEED_SECTION_SENTENCES,
...
function markdownFor(report) {
  const lines = [
    '# Report Contract Doc Page Bootstrap Seed Section Regression',
    '',
    `Status: ${report.status}`,
    `Hash: ${report.contractDocPageBootstrapSeedSectionRegressionHash}`,
    `Generated: ${report.generatedAt}`,
    '',
    '## Summary',
    '',
    `- Actual doc page bootstrap seed sections ok: ${report.summary.actualOk}`,
    ...
    `- Bootstrap seed sections: ${report.summary.sectionCount}/${report.summary.contractCount}`,
    ...REPORT_CONTRACT_DOC_PAGE_BOOTSTRAP_SEED_SECTION_SENTENCES.map((binding) => countLine(report, binding)),
    ...
    '## Contracts',
    '',
    `| Contract | Docs | Section | ${REPORT_CONTRACT_DOC_PAGE_BOOTSTRAP_SEED_SECTION_SENTENCES.map((binding) => binding.label).join(' | ')} | Order |`,
    `| --- | --- | --- | ${REPORT_CONTRACT_DOC_PAGE_BOOTSTRAP_SEED_SECTION_SENTENCES.map(() => '---').join(' | ')} | --- |`,
    ...report.actual.contracts.map((contract) => `| ${contract.contractId} | ${contract.docsPath} | ${contract.sectionPresent} | ${REPORT_CONTRACT_DOC_PAGE_BOOTSTRAP_SEED_SECTION_SENTENCES.map((binding) => contractCell(contract, binding)).join(' | ')} | ${contract.orderValid} |`),
    '',
    '## Scenarios',
    '',
    ...
    '- Synthetic fixture proves missing bootstrap-seed bindings and order drift fail closed.',
...
export function buildReportContractDocPageBootstrapSeedSectionRegressionLatest({
  generatedAt = new Date().toISOString(),
} = {}) {
  return buildReportContractDocPageBootstrapSeedSectionRegressionReport({
...
function main() {
  ...
  const report = buildReportContractDocPageBootstrapSeedSectionRegressionLatest();
  const reportFiles = writeReports(report);
  process.stdout.write(`${JSON.stringify({
    ok: report.ok,
    status: report.status,
    contractDocPageBootstrapSeedSectionRegressionHash: report.contractDocPageBootstrapSeedSectionRegressionHash,
