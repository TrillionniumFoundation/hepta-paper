...
import {
  REPORT_CONTRACT_DOC_PAGE_CHECKPOINT_HASH_SECTION_SENTENCES,
...
function markdownFor(report) {
  const lines = [
    '# Report Contract Doc Page Checkpoint Hash Section Regression',
    '',
    `Status: ${report.status}`,
    `Hash: ${report.contractDocPageCheckpointHashSectionRegressionHash}`,
    `Generated: ${report.generatedAt}`,
    '',
    '## Summary',
    '',
    `- Actual doc page checkpoint hash sections ok: ${report.summary.actualOk}`,
    ...
    `- Checkpoint hash sections: ${report.summary.sectionCount}/${report.summary.contractCount}`,
    ...REPORT_CONTRACT_DOC_PAGE_CHECKPOINT_HASH_SECTION_SENTENCES.map((binding) => countLine(report, binding)),
    ...
    '## Contracts',
    '',
    `| Contract | Docs | Section | ${REPORT_CONTRACT_DOC_PAGE_CHECKPOINT_HASH_SECTION_SENTENCES.map((binding) => binding.label).join(' | ')} | Order |`,
    `| --- | --- | --- | ${REPORT_CONTRACT_DOC_PAGE_CHECKPOINT_HASH_SECTION_SENTENCES.map(() => '---').join(' | ')} | --- |`,
    ...report.actual.contracts.map((contract) => `| ${contract.contractId} | ${contract.docsPath} | ${contract.sectionPresent} | ${REPORT_CONTRACT_DOC_PAGE_CHECKPOINT_HASH_SECTION_SENTENCES.map((binding) => contractCell(contract, binding)).join(' | ')} | ${contract.orderValid} |`),
    '',
    '## Scenarios',
    '',
    ...
    '- Synthetic fixture proves missing checkpoint/hash bindings and order drift fail closed.',
...
export function buildReportContractDocPageCheckpointHashSectionRegressionLatest({
  generatedAt = new Date().toISOString(),
} = {}) {
  return buildReportContractDocPageCheckpointHashSectionRegressionReport({
...
function main() {
  ...
  const report = buildReportContractDocPageCheckpointHashSectionRegressionLatest();
  const reportFiles = writeReports(report);
  process.stdout.write(`${JSON.stringify({
    ok: report.ok,
    status: report.status,
    contractDocPageCheckpointHashSectionRegressionHash: report.contractDocPageCheckpointHashSectionRegressionHash,
