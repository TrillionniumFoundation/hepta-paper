...
import {
  REPORT_CONTRACT_DOC_PAGE_CLEAN_RERUN_SECTION_SENTENCES,
...
function markdownFor(report) {
  const lines = [
    '# Report Contract Doc Page Clean Rerun Section Regression',
    '',
    `Status: ${report.status}`,
    `Hash: ${report.contractDocPageCleanRerunSectionRegressionHash}`,
    `Generated: ${report.generatedAt}`,
    '',
    '## Summary',
    '',
    `- Actual doc page clean rerun sections ok: ${report.summary.actualOk}`,
    ...
    `- Clean rerun sections: ${report.summary.sectionCount}/${report.summary.contractCount}`,
    ...REPORT_CONTRACT_DOC_PAGE_CLEAN_RERUN_SECTION_SENTENCES.map((binding) => countLine(report, binding)),
    ...
    '## Contracts',
    '',
    `| Contract | Docs | Section | ${REPORT_CONTRACT_DOC_PAGE_CLEAN_RERUN_SECTION_SENTENCES.map((binding) => binding.label).join(' | ')} | Order |`,
    `| --- | --- | --- | ${REPORT_CONTRACT_DOC_PAGE_CLEAN_RERUN_SECTION_SENTENCES.map(() => '---').join(' | ')} | --- |`,
    ...report.actual.contracts.map((contract) => `| ${contract.contractId} | ${contract.docsPath} | ${contract.sectionPresent} | ${REPORT_CONTRACT_DOC_PAGE_CLEAN_RERUN_SECTION_SENTENCES.map((binding) => contractCell(contract, binding)).join(' | ')} | ${contract.orderValid} |`),
    '',
    '## Scenarios',
    '',
    ...
    '- Synthetic fixture proves missing clean-rerun bindings and order drift fail closed.',
...
export function buildReportContractDocPageCleanRerunSectionRegressionLatest({
  generatedAt = new Date().toISOString(),
} = {}) {
  return buildReportContractDocPageCleanRerunSectionRegressionReport({
...
function main() {
  ...
  const report = buildReportContractDocPageCleanRerunSectionRegressionLatest();
  const reportFiles = writeReports(report);
  process.stdout.write(`${JSON.stringify({
    ok: report.ok,
    status: report.status,
    contractDocPageCleanRerunSectionRegressionHash: report.contractDocPageCleanRerunSectionRegressionHash,
