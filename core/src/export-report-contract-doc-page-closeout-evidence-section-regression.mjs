...
import {
  REPORT_CONTRACT_DOC_PAGE_CLOSEOUT_EVIDENCE_SECTION_SENTENCES,
...
function markdownFor(report) {
  const lines = [
    '# Report Contract Doc Page Closeout Evidence Section Regression',
    '',
    `Status: ${report.status}`,
    `Hash: ${report.contractDocPageCloseoutEvidenceSectionRegressionHash}`,
    `Generated: ${report.generatedAt}`,
    '',
    '## Summary',
    '',
    `- Actual doc page closeout evidence sections ok: ${report.summary.actualOk}`,
    ...
    `- Closeout evidence sections: ${report.summary.sectionCount}/${report.summary.contractCount}`,
    ...REPORT_CONTRACT_DOC_PAGE_CLOSEOUT_EVIDENCE_SECTION_SENTENCES.map((binding) => countLine(report, binding)),
    ...
    '## Contracts',
    '',
    `| Contract | Docs | Section | ${REPORT_CONTRACT_DOC_PAGE_CLOSEOUT_EVIDENCE_SECTION_SENTENCES.map((binding) => binding.label).join(' | ')} | Order |`,
    `| --- | --- | --- | ${REPORT_CONTRACT_DOC_PAGE_CLOSEOUT_EVIDENCE_SECTION_SENTENCES.map(() => '---').join(' | ')} | --- |`,
    ...report.actual.contracts.map((contract) => `| ${contract.contractId} | ${contract.docsPath} | ${contract.sectionPresent} | ${REPORT_CONTRACT_DOC_PAGE_CLOSEOUT_EVIDENCE_SECTION_SENTENCES.map((binding) => contractCell(contract, binding)).join(' | ')} | ${contract.orderValid} |`),
    '',
    '## Scenarios',
    '',
    ...
    '- Synthetic fixture proves missing closeout-evidence bindings and order drift fail closed.',
...
export function buildReportContractDocPageCloseoutEvidenceSectionRegressionLatest({
  generatedAt = new Date().toISOString(),
} = {}) {
  return buildReportContractDocPageCloseoutEvidenceSectionRegressionReport({
...
function main() {
  ...
  const report = buildReportContractDocPageCloseoutEvidenceSectionRegressionLatest();
  const reportFiles = writeReports(report);
  process.stdout.write(`${JSON.stringify({
    ok: report.ok,
    status: report.status,
    contractDocPageCloseoutEvidenceSectionRegressionHash: report.contractDocPageCloseoutEvidenceSectionRegressionHash,
