...
import {
  REPORT_CONTRACT_DOC_PAGE_CLOSEOUT_RELEASE_MANIFEST_SECTION_SENTENCES,
...
function markdownFor(report) {
  const lines = [
    '# Report Contract Doc Page Closeout Release Manifest Section Regression',
    '',
    `Status: ${report.status}`,
    `Hash: ${report.contractDocPageCloseoutReleaseManifestSectionRegressionHash}`,
    `Generated: ${report.generatedAt}`,
    '',
    '## Summary',
    '',
    `- Actual doc page closeout release manifest sections ok: ${report.summary.actualOk}`,
    ...
    `- Closeout release manifest sections: ${report.summary.sectionCount}/${report.summary.contractCount}`,
    ...REPORT_CONTRACT_DOC_PAGE_CLOSEOUT_RELEASE_MANIFEST_SECTION_SENTENCES.map((binding) => countLine(report, binding)),
    ...
    '## Contracts',
    '',
    `| Contract | Docs | Section | ${REPORT_CONTRACT_DOC_PAGE_CLOSEOUT_RELEASE_MANIFEST_SECTION_SENTENCES.map((binding) => binding.label).join(' | ')} | Order |`,
    `| --- | --- | --- | ${REPORT_CONTRACT_DOC_PAGE_CLOSEOUT_RELEASE_MANIFEST_SECTION_SENTENCES.map(() => '---').join(' | ')} | --- |`,
    ...report.actual.contracts.map((contract) => `| ${contract.contractId} | ${contract.docsPath} | ${contract.sectionPresent} | ${REPORT_CONTRACT_DOC_PAGE_CLOSEOUT_RELEASE_MANIFEST_SECTION_SENTENCES.map((binding) => contractCell(contract, binding)).join(' | ')} | ${contract.orderValid} |`),
    '',
    '## Scenarios',
    '',
    ...
    '- Synthetic fixture proves missing closeout-release-manifest bindings and order drift fail closed.',
...
export function buildReportContractDocPageCloseoutReleaseManifestSectionRegressionLatest({
  generatedAt = new Date().toISOString(),
} = {}) {
  return buildReportContractDocPageCloseoutReleaseManifestSectionRegressionReport({
...
function main() {
  ...
  const report = buildReportContractDocPageCloseoutReleaseManifestSectionRegressionLatest();
  const reportFiles = writeReports(report);
  process.stdout.write(`${JSON.stringify({
    ok: report.ok,
    status: report.status,
    contractDocPageCloseoutReleaseManifestSectionRegressionHash: report.contractDocPageCloseoutReleaseManifestSectionRegressionHash,
