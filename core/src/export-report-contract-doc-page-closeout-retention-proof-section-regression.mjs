...
import {
  REPORT_CONTRACT_DOC_PAGE_CLOSEOUT_RETENTION_PROOF_SECTION_SENTENCES,
...
function markdownFor(report) {
  const lines = [
    '# Report Contract Doc Page Closeout Retention Proof Section Regression',
    '',
    `Status: ${report.status}`,
    `Hash: ${report.contractDocPageCloseoutRetentionProofSectionRegressionHash}`,
    `Generated: ${report.generatedAt}`,
    '',
    '## Summary',
    '',
    `- Actual doc page closeout retention proof sections ok: ${report.summary.actualOk}`,
    ...
    `- Closeout retention proof sections: ${report.summary.sectionCount}/${report.summary.contractCount}`,
    ...REPORT_CONTRACT_DOC_PAGE_CLOSEOUT_RETENTION_PROOF_SECTION_SENTENCES.map((binding) => countLine(report, binding)),
    ...
    '## Contracts',
    '',
    `| Contract | Docs | Section | ${REPORT_CONTRACT_DOC_PAGE_CLOSEOUT_RETENTION_PROOF_SECTION_SENTENCES.map((binding) => binding.label).join(' | ')} | Order |`,
    `| --- | --- | --- | ${REPORT_CONTRACT_DOC_PAGE_CLOSEOUT_RETENTION_PROOF_SECTION_SENTENCES.map(() => '---').join(' | ')} | --- |`,
    ...report.actual.contracts.map((contract) => `| ${contract.contractId} | ${contract.docsPath} | ${contract.sectionPresent} | ${REPORT_CONTRACT_DOC_PAGE_CLOSEOUT_RETENTION_PROOF_SECTION_SENTENCES.map((binding) => contractCell(contract, binding)).join(' | ')} | ${contract.orderValid} |`),
    '',
    '## Scenarios',
    '',
    ...
    '- Synthetic fixture proves missing closeout-retention-proof bindings and order drift fail closed.',
...
export function buildReportContractDocPageCloseoutRetentionProofSectionRegressionLatest({
  generatedAt = new Date().toISOString(),
} = {}) {
  return buildReportContractDocPageCloseoutRetentionProofSectionRegressionReport({
...
function main() {
  ...
  const report = buildReportContractDocPageCloseoutRetentionProofSectionRegressionLatest();
  const reportFiles = writeReports(report);
  process.stdout.write(`${JSON.stringify({
    ok: report.ok,
    status: report.status,
    contractDocPageCloseoutRetentionProofSectionRegressionHash: report.contractDocPageCloseoutRetentionProofSectionRegressionHash,
