...
function markdownFor(report) {
  const lines = [
    '# Report Contract Doc Page Command Section Regression',
    '',
    `Status: ${report.status}`,
    `Hash: ${report.contractDocPageCommandSectionRegressionHash}`,
    `Generated: ${report.generatedAt}`,
    '',
    '## Summary',
    '',
    `- Actual doc page command sections ok: ${report.summary.actualOk}`,
    ...
    `- Command sections: ${report.summary.sectionCount}/${report.summary.contractCount}`,
    `- Commands: ${report.summary.commandCount}/${report.summary.contractCount}`,
    ...
    `- Strict-gate sentences: ${report.summary.strictGateCount}/${report.summary.contractCount}`,
    `- Safety sentences: ${report.summary.safetyCount}/${report.summary.contractCount}`,
    ...
    '## Contracts',
    '',
    '| Contract | Docs | Section | Command | Latest JSON | Latest Markdown | Strict | Safety | Order |',
    '| --- | --- | --- | --- | --- | --- | --- | --- | --- |',
    ...report.actual.contracts.map((contract) => `| ${contract.contractId} | ${contract.docsPath} | ${contract.sectionPresent} | ${contract.commandPresent} | ${contract.latestJsonPresent} | ${contract.latestMarkdownPresent} | ${contract.strictGatePresent} | ${contract.safetyPresent} | ${contract.orderValid} |`),
    '',
    '## Scenarios',
    '',
    ...
    ...(report.blockers.length
      ? report.blockers.map((item) => `- ${item.code} ${item.contractId || item.scenarioId || ''} ${item.docsPath || item.scriptId || item.fileId || item.mdFileId || ''}: ${item.notes}`.trim())
      : ['- none']),
    '',
    '## Safety',
    '',
    '- Reads local docs pages and the report contract manifest.',
    '- Synthetic fixture proves missing command, latest artifacts, strict-gate, safety, and order drift fail closed.',
...
export function buildReportContractDocPageCommandSectionRegressionLatest({
  generatedAt = new Date().toISOString(),
} = {}) {
  return buildReportContractDocPageCommandSectionRegressionReport({
...
function main() {
  ...
  const report = buildReportContractDocPageCommandSectionRegressionLatest();
  const reportFiles = writeReports(report);
  process.stdout.write(`${JSON.stringify({
    ok: report.ok,
    status: report.status,
    contractDocPageCommandSectionRegressionHash: report.contractDocPageCommandSectionRegressionHash,
