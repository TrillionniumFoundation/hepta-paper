...
import {
  REPORT_CONTRACT_DOC_PAGE_CROSS_REPORT_SECTION_SENTENCES,
...
function markdownFor(report) {
  const lines = [
    '# Report Contract Doc Page Cross-report Section Regression',
    '',
    `Status: ${report.status}`,
    `Hash: ${report.contractDocPageCrossReportSectionRegressionHash}`,
    `Generated: ${report.generatedAt}`,
    '',
    '## Summary',
    '',
    `- Actual doc page cross-report sections ok: ${report.summary.actualOk}`,
    ...
    `- Cross-report sections: ${report.summary.sectionCount}/${report.summary.contractCount}`,
    ...REPORT_CONTRACT_DOC_PAGE_CROSS_REPORT_SECTION_SENTENCES.map((binding) => countLine(report, binding)),
    ...
    '## Contracts',
    '',
    `| Contract | Docs | Section | ${REPORT_CONTRACT_DOC_PAGE_CROSS_REPORT_SECTION_SENTENCES.map((binding) => binding.label).join(' | ')} | Order |`,
    `| --- | --- | --- | ${REPORT_CONTRACT_DOC_PAGE_CROSS_REPORT_SECTION_SENTENCES.map(() => '---').join(' | ')} | --- |`,
    ...report.actual.contracts.map((contract) => `| ${contract.contractId} | ${contract.docsPath} | ${contract.sectionPresent} | ${REPORT_CONTRACT_DOC_PAGE_CROSS_REPORT_SECTION_SENTENCES.map((binding) => contractCell(contract, binding)).join(' | ')} | ${contract.orderValid} |`),
    '',
    '## Scenarios',
    '',
    ...
    '- Synthetic fixture proves missing cross-report visibility bindings and order drift fail closed.',
...
export function buildReportContractDocPageCrossReportSectionRegressionLatest({
  generatedAt = new Date().toISOString(),
} = {}) {
  return buildReportContractDocPageCrossReportSectionRegressionReport({
...
function main() {
  ...
  const report = buildReportContractDocPageCrossReportSectionRegressionLatest();
  const reportFiles = writeReports(report);
  process.stdout.write(`${JSON.stringify({
    ok: report.ok,
    status: report.status,
    contractDocPageCrossReportSectionRegressionHash: report.contractDocPageCrossReportSectionRegressionHash,
