...
import {
  REPORT_CONTRACT_DOC_PAGE_FINAL_SETTLEMENT_SECTION_SENTENCES,
...
function markdownFor(report) {
  const lines = [
    '# Report Contract Doc Page Final Settlement Section Regression',
    '',
    `Status: ${report.status}`,
    `Hash: ${report.contractDocPageFinalSettlementSectionRegressionHash}`,
    `Generated: ${report.generatedAt}`,
    '',
    '## Summary',
    '',
    `- Actual doc page final settlement sections ok: ${report.summary.actualOk}`,
    ...
    `- Final settlement sections: ${report.summary.sectionCount}/${report.summary.contractCount}`,
    ...REPORT_CONTRACT_DOC_PAGE_FINAL_SETTLEMENT_SECTION_SENTENCES.map((binding) => countLine(report, binding)),
    ...
    '## Contracts',
    '',
    `| Contract | Docs | Section | ${REPORT_CONTRACT_DOC_PAGE_FINAL_SETTLEMENT_SECTION_SENTENCES.map((binding) => binding.label).join(' | ')} | Order |`,
    `| --- | --- | --- | ${REPORT_CONTRACT_DOC_PAGE_FINAL_SETTLEMENT_SECTION_SENTENCES.map(() => '---').join(' | ')} | --- |`,
    ...report.actual.contracts.map((contract) => `| ${contract.contractId} | ${contract.docsPath} | ${contract.sectionPresent} | ${REPORT_CONTRACT_DOC_PAGE_FINAL_SETTLEMENT_SECTION_SENTENCES.map((binding) => contractCell(contract, binding)).join(' | ')} | ${contract.orderValid} |`),
    '',
    '## Scenarios',
    '',
    ...
    '- Synthetic fixture proves missing final-settlement bindings and order drift fail closed.',
...
export function buildReportContractDocPageFinalSettlementSectionRegressionLatest({
  generatedAt = new Date().toISOString(),
} = {}) {
  return buildReportContractDocPageFinalSettlementSectionRegressionReport({
...
function main() {
  ...
  const report = buildReportContractDocPageFinalSettlementSectionRegressionLatest();
  const reportFiles = writeReports(report);
  process.stdout.write(`${JSON.stringify({
    ok: report.ok,
    status: report.status,
    contractDocPageFinalSettlementSectionRegressionHash: report.contractDocPageFinalSettlementSectionRegressionHash,
