...
import {
  REPORT_CONTRACT_DOC_PAGE_FRESHNESS_HASH_SECTION_SENTENCES,
...
function markdownFor(report) {
  const lines = [
    '# Report Contract Doc Page Freshness Hash Section Regression',
    '',
    `Status: ${report.status}`,
    `Hash: ${report.contractDocPageFreshnessHashSectionRegressionHash}`,
    `Generated: ${report.generatedAt}`,
    '',
    '## Summary',
    '',
    `- Actual doc page freshness hash sections ok: ${report.summary.actualOk}`,
    ...
    `- Freshness hash sections: ${report.summary.sectionCount}/${report.summary.contractCount}`,
    ...REPORT_CONTRACT_DOC_PAGE_FRESHNESS_HASH_SECTION_SENTENCES.map((binding) => countLine(report, binding)),
    ...
    '## Contracts',
    '',
    `| Contract | Docs | Section | ${REPORT_CONTRACT_DOC_PAGE_FRESHNESS_HASH_SECTION_SENTENCES.map((binding) => binding.label).join(' | ')} | Order |`,
    `| --- | --- | --- | ${REPORT_CONTRACT_DOC_PAGE_FRESHNESS_HASH_SECTION_SENTENCES.map(() => '---').join(' | ')} | --- |`,
    ...report.actual.contracts.map((contract) => `| ${contract.contractId} | ${contract.docsPath} | ${contract.sectionPresent} | ${REPORT_CONTRACT_DOC_PAGE_FRESHNESS_HASH_SECTION_SENTENCES.map((binding) => contractCell(contract, binding)).join(' | ')} | ${contract.orderValid} |`),
    '',
    '## Scenarios',
    '',
    ...
    '- Synthetic fixture proves missing freshness/hash bindings and order drift fail closed.',
...
export function buildReportContractDocPageFreshnessHashSectionRegressionLatest({
  generatedAt = new Date().toISOString(),
} = {}) {
  return buildReportContractDocPageFreshnessHashSectionRegressionReport({
...
function main() {
  ...
  const report = buildReportContractDocPageFreshnessHashSectionRegressionLatest();
  const reportFiles = writeReports(report);
  process.stdout.write(`${JSON.stringify({
    ok: report.ok,
    status: report.status,
    contractDocPageFreshnessHashSectionRegressionHash: report.contractDocPageFreshnessHashSectionRegressionHash,
