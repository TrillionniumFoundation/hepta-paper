...
function markdownFor(report) {
  const lines = [
    '# Report Contract Doc Page Output Section Regression',
    '',
    `Status: ${report.status}`,
    `Hash: ${report.contractDocPageOutputSectionRegressionHash}`,
    `Generated: ${report.generatedAt}`,
    '',
    '## Summary',
    '',
    `- Actual doc page output sections ok: ${report.summary.actualOk}`,
    ...
    `- Output sections: ${report.summary.sectionCount}/${report.summary.contractCount}`,
    `- JSON outputs: ${report.summary.jsonOutputCount}/${report.summary.contractCount}`,
    `- Markdown outputs: ${report.summary.markdownOutputCount}/${report.summary.contractCount}`,
    `- README/reports README bindings: ${report.summary.indexBindingCount}/${report.summary.contractCount}`,
    `- Cross-report bindings: ${report.summary.crossReportBindingCount}/${report.summary.contractCount}`,
    ...
    '## Contracts',
    '',
    '| Contract | Docs | Section | JSON | Markdown | Index | Cross-report | Order |',
    '| --- | --- | --- | --- | --- | --- | --- | --- |',
    ...report.actual.contracts.map((contract) => `| ${contract.contractId} | ${contract.docsPath} | ${contract.sectionPresent} | ${contract.jsonOutputPresent} | ${contract.markdownOutputPresent} | ${contract.indexBindingPresent} | ${contract.crossReportBindingPresent} | ${contract.orderValid} |`),
    '',
    '## Scenarios',
    '',
    ...
    '- Synthetic fixture proves missing latest output paths, README/reports README bindings, cross-report bindings, and order drift fail closed.',
...
export function buildReportContractDocPageOutputSectionRegressionLatest({
  generatedAt = new Date().toISOString(),
} = {}) {
  return buildReportContractDocPageOutputSectionRegressionReport({
...
function main() {
  ...
  const report = buildReportContractDocPageOutputSectionRegressionLatest();
  const reportFiles = writeReports(report);
  process.stdout.write(`${JSON.stringify({
    ok: report.ok,
    status: report.status,
    contractDocPageOutputSectionRegressionHash: report.contractDocPageOutputSectionRegressionHash,
