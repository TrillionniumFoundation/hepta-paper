...
import {
  REPORT_CONTRACT_DOC_PAGE_POST_GATE_WRITER_SECTION_SENTENCES,
...
function markdownFor(report) {
  const lines = [
    '# Report Contract Doc Page Post-Gate Writer Section Regression',
    '',
    `Status: ${report.status}`,
    `Hash: ${report.contractDocPagePostGateWriterSectionRegressionHash}`,
    `Generated: ${report.generatedAt}`,
    '',
    '## Summary',
    '',
    `- Actual doc page post-gate writer sections ok: ${report.summary.actualOk}`,
    ...
    `- Post-gate writer sections: ${report.summary.sectionCount}/${report.summary.contractCount}`,
    ...REPORT_CONTRACT_DOC_PAGE_POST_GATE_WRITER_SECTION_SENTENCES.map((binding) => countLine(report, binding)),
    ...
    '## Contracts',
    '',
    `| Contract | Docs | Section | ${REPORT_CONTRACT_DOC_PAGE_POST_GATE_WRITER_SECTION_SENTENCES.map((binding) => binding.label).join(' | ')} | Order |`,
    `| --- | --- | --- | ${REPORT_CONTRACT_DOC_PAGE_POST_GATE_WRITER_SECTION_SENTENCES.map(() => '---').join(' | ')} | --- |`,
    ...report.actual.contracts.map((contract) => `| ${contract.contractId} | ${contract.docsPath} | ${contract.sectionPresent} | ${REPORT_CONTRACT_DOC_PAGE_POST_GATE_WRITER_SECTION_SENTENCES.map((binding) => contractCell(contract, binding)).join(' | ')} | ${contract.orderValid} |`),
    '',
    '## Scenarios',
    '',
    ...
    '- Synthetic fixture proves missing post-gate writer bindings and order drift fail closed.',
...
export function buildReportContractDocPagePostGateWriterSectionRegressionLatest({
  generatedAt = new Date().toISOString(),
} = {}) {
  return buildReportContractDocPagePostGateWriterSectionRegressionReport({
...
function main() {
  ...
  const report = buildReportContractDocPagePostGateWriterSectionRegressionLatest();
  const reportFiles = writeReports(report);
  process.stdout.write(`${JSON.stringify({
    ok: report.ok,
    status: report.status,
    contractDocPagePostGateWriterSectionRegressionHash: report.contractDocPagePostGateWriterSectionRegressionHash,
