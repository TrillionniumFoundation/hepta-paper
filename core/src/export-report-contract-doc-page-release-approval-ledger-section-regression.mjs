...
import {
  REPORT_CONTRACT_DOC_PAGE_RELEASE_APPROVAL_LEDGER_SECTION_SENTENCES,
...
function markdownFor(report) {
  const lines = [
    ...
    `Hash: ${report.contractDocPageReleaseApprovalLedgerSectionRegressionHash}`,
    `Generated: ${report.generatedAt}`,
    '',
    '## Summary',
    '',
    `- Actual doc page release approval ledger sections ok: ${report.summary.actualOk}`,
    ...
    `- Release approval ledger sections: ${report.summary.sectionCount}/${report.summary.contractCount}`,
    ...REPORT_CONTRACT_DOC_PAGE_RELEASE_APPROVAL_LEDGER_SECTION_SENTENCES.map((binding) => countLine(report, binding)),
    ...
    '## Contracts',
    '',
    `| Contract | Docs | Section | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_APPROVAL_LEDGER_SECTION_SENTENCES.map((binding) => binding.label).join(' | ')} | Order |`,
    `| --- | --- | --- | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_APPROVAL_LEDGER_SECTION_SENTENCES.map(() => '---').join(' | ')} | --- |`,
    ...report.actual.contracts.map((contract) => `| ${contract.contractId} | ${contract.docsPath} | ${contract.sectionPresent} | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_APPROVAL_LEDGER_SECTION_SENTENCES.map((binding) => contractCell(contract, binding)).join(' | ')} | ${contract.orderValid} |`),
    '',
    '## Scenarios',
    '',
    ...
    '- Synthetic fixture proves missing release approval ledger bindings, ledger order drift, and shared-doc mapping drift fail closed.',
...
export function buildReportContractDocPageReleaseApprovalLedgerSectionRegressionLatest({
  generatedAt = new Date().toISOString(),
} = {}) {
  return buildReportContractDocPageReleaseApprovalLedgerSectionRegressionReport({
...
function main() {
  ...
  const report = buildReportContractDocPageReleaseApprovalLedgerSectionRegressionLatest();
  const reportFiles = writeReports(report);
  process.stdout.write(`${JSON.stringify({
    ok: report.ok,
    status: report.status,
    contractDocPageReleaseApprovalLedgerSectionRegressionHash: report.contractDocPageReleaseApprovalLedgerSectionRegressionHash,
