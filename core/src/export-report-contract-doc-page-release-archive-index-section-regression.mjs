...
import {
  REPORT_CONTRACT_DOC_PAGE_RELEASE_ARCHIVE_INDEX_SECTION_SENTENCES,
...
function markdownFor(report) {
  const lines = [
    ...
    `Hash: ${report.contractDocPageReleaseArchiveIndexSectionRegressionHash}`,
    `Generated: ${report.generatedAt}`,
    '',
    '## Summary',
    '',
    `- Actual doc page release archive index sections ok: ${report.summary.actualOk}`,
    ...
    ...REPORT_CONTRACT_DOC_PAGE_RELEASE_ARCHIVE_INDEX_SECTION_SENTENCES.map((binding) => countLine(report, binding)),
    ...
    '## Contracts',
    '',
    `| Contract | Docs | Section | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_ARCHIVE_INDEX_SECTION_SENTENCES.map((binding) => binding.label).join(' | ')} | Order |`,
    `| --- | --- | --- | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_ARCHIVE_INDEX_SECTION_SENTENCES.map(() => '---').join(' | ')} | --- |`,
    ...report.actual.contracts.map((contract) => `| ${contract.contractId} | ${contract.docsPath} | ${contract.sectionPresent} | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_ARCHIVE_INDEX_SECTION_SENTENCES.map((binding) => contractCell(contract, binding)).join(' | ')} | ${contract.orderValid} |`),
    '',
    '## Scenarios',
    '',
    ...
    '- Synthetic fixture proves missing release-archive-index bindings and order drift fail closed.',
...
export function buildReportContractDocPageReleaseArchiveIndexSectionRegressionLatest({
  generatedAt = new Date().toISOString(),
} = {}) {
  return buildReportContractDocPageReleaseArchiveIndexSectionRegressionReport({
...
function main() {
  ...
  const report = buildReportContractDocPageReleaseArchiveIndexSectionRegressionLatest();
  const reportFiles = writeReports(report);
  process.stdout.write(`${JSON.stringify({
    ok: report.ok,
    status: report.status,
    contractDocPageReleaseArchiveIndexSectionRegressionHash: report.contractDocPageReleaseArchiveIndexSectionRegressionHash,
