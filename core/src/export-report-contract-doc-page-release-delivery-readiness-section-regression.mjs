...
import {
  REPORT_CONTRACT_DOC_PAGE_RELEASE_DELIVERY_READINESS_SECTION_SENTENCES,
...
function markdownFor(report) {
  const lines = [
    ...
    `Hash: ${report.contractDocPageReleaseDeliveryReadinessSectionRegressionHash}`,
    `Generated: ${report.generatedAt}`,
    '',
    '## Summary',
    '',
    `- Actual doc page release delivery readiness sections ok: ${report.summary.actualOk}`,
    ...
    `- Release delivery readiness sections: ${report.summary.sectionCount}/${report.summary.contractCount}`,
    ...REPORT_CONTRACT_DOC_PAGE_RELEASE_DELIVERY_READINESS_SECTION_SENTENCES.map((binding) => countLine(report, binding)),
    ...
    '## Contracts',
    '',
    `| Contract | Docs | Section | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_DELIVERY_READINESS_SECTION_SENTENCES.map((binding) => binding.label).join(' | ')} | Order |`,
    `| --- | --- | --- | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_DELIVERY_READINESS_SECTION_SENTENCES.map(() => '---').join(' | ')} | --- |`,
    ...report.actual.contracts.map((contract) => `| ${contract.contractId} | ${contract.docsPath} | ${contract.sectionPresent} | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_DELIVERY_READINESS_SECTION_SENTENCES.map((binding) => contractCell(contract, binding)).join(' | ')} | ${contract.orderValid} |`),
    '',
    '## Scenarios',
    '',
    ...
    '- Synthetic fixture proves missing release-delivery-readiness bindings and order drift fail closed.',
...
export function buildReportContractDocPageReleaseDeliveryReadinessSectionRegressionLatest({
  generatedAt = new Date().toISOString(),
} = {}) {
  return buildReportContractDocPageReleaseDeliveryReadinessSectionRegressionReport({
...
function main() {
  ...
  const report = buildReportContractDocPageReleaseDeliveryReadinessSectionRegressionLatest();
  const reportFiles = writeReports(report);
  process.stdout.write(`${JSON.stringify({
    ok: report.ok,
    status: report.status,
    contractDocPageReleaseDeliveryReadinessSectionRegressionHash: report.contractDocPageReleaseDeliveryReadinessSectionRegressionHash,
