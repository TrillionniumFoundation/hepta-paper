...
import {
  REPORT_CONTRACT_DOC_PAGE_RELEASE_DISPATCH_IMPLEMENTATION_DENIAL_SECTION_SENTENCES,
...
function markdownFor(report) {
  const lines = [
    ...
    `Hash: ${report.contractDocPageReleaseDispatchImplementationDenialSectionRegressionHash}`,
    `Generated: ${report.generatedAt}`,
    '',
    '## Summary',
    '',
    `- Actual doc page release dispatch implementation denial sections ok: ${report.summary.actualOk}`,
    ...
    `- Release dispatch implementation denial sections: ${report.summary.sectionCount}/${report.summary.contractCount}`,
    ...REPORT_CONTRACT_DOC_PAGE_RELEASE_DISPATCH_IMPLEMENTATION_DENIAL_SECTION_SENTENCES.map((binding) => countLine(report, binding)),
    ...
    '## Contracts',
    '',
    `| Contract | Docs | Section | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_DISPATCH_IMPLEMENTATION_DENIAL_SECTION_SENTENCES.map((binding) => binding.label).join(' | ')} | Order |`,
    `| --- | --- | --- | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_DISPATCH_IMPLEMENTATION_DENIAL_SECTION_SENTENCES.map(() => '---').join(' | ')} | --- |`,
    ...report.actual.contracts.map((contract) => `| ${contract.contractId} | ${contract.docsPath} | ${contract.sectionPresent} | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_DISPATCH_IMPLEMENTATION_DENIAL_SECTION_SENTENCES.map((binding) => contractCell(contract, binding)).join(' | ')} | ${contract.orderValid} |`),
    '',
    '## Scenarios',
    '',
    ...
    '- Synthetic fixture proves missing release dispatch implementation denial bindings, dispatch-implementation order drift, and shared-doc mapping drift fail closed.',
...
export function buildReportContractDocPageReleaseDispatchImplementationDenialSectionRegressionLatest({
  generatedAt = new Date().toISOString(),
} = {}) {
  return buildReportContractDocPageReleaseDispatchImplementationDenialSectionRegressionReport({
...
function main() {
  ...
  const report = buildReportContractDocPageReleaseDispatchImplementationDenialSectionRegressionLatest();
  const reportFiles = writeReports(report);
  process.stdout.write(`${JSON.stringify({
    ok: report.ok,
    status: report.status,
    contractDocPageReleaseDispatchImplementationDenialSectionRegressionHash: report.contractDocPageReleaseDispatchImplementationDenialSectionRegressionHash,
