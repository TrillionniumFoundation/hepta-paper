...
import {
  REPORT_CONTRACT_DOC_PAGE_RELEASE_EXECUTION_APPROVAL_BOUNDARY_SECTION_SENTENCES,
...
function markdownFor(report) {
  const lines = [
    ...
    `Hash: ${report.contractDocPageReleaseExecutionApprovalBoundarySectionRegressionHash}`,
    `Generated: ${report.generatedAt}`,
    '',
    '## Summary',
    '',
    `- Actual doc page release execution approval boundary sections ok: ${report.summary.actualOk}`,
    ...
    `- Release execution approval boundary sections: ${report.summary.sectionCount}/${report.summary.contractCount}`,
    ...REPORT_CONTRACT_DOC_PAGE_RELEASE_EXECUTION_APPROVAL_BOUNDARY_SECTION_SENTENCES.map((binding) => countLine(report, binding)),
    ...
    '## Contracts',
    '',
    `| Contract | Docs | Section | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_EXECUTION_APPROVAL_BOUNDARY_SECTION_SENTENCES.map((binding) => binding.label).join(' | ')} | Order |`,
    `| --- | --- | --- | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_EXECUTION_APPROVAL_BOUNDARY_SECTION_SENTENCES.map(() => '---').join(' | ')} | --- |`,
    ...report.actual.contracts.map((contract) => `| ${contract.contractId} | ${contract.docsPath} | ${contract.sectionPresent} | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_EXECUTION_APPROVAL_BOUNDARY_SECTION_SENTENCES.map((binding) => contractCell(contract, binding)).join(' | ')} | ${contract.orderValid} |`),
    '',
    '## Scenarios',
    '',
    ...
    '- Synthetic fixture proves missing release execution approval boundary bindings, approval-boundary order drift, and shared-doc mapping drift fail closed.',
...
export function buildReportContractDocPageReleaseExecutionApprovalBoundarySectionRegressionLatest({
  generatedAt = new Date().toISOString(),
} = {}) {
  return buildReportContractDocPageReleaseExecutionApprovalBoundarySectionRegressionReport({
...
function main() {
  ...
  const report = buildReportContractDocPageReleaseExecutionApprovalBoundarySectionRegressionLatest();
  const reportFiles = writeReports(report);
  process.stdout.write(`${JSON.stringify({
    ok: report.ok,
    status: report.status,
    contractDocPageReleaseExecutionApprovalBoundarySectionRegressionHash: report.contractDocPageReleaseExecutionApprovalBoundarySectionRegressionHash,
