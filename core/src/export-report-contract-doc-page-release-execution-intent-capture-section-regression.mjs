...
import {
  REPORT_CONTRACT_DOC_PAGE_RELEASE_EXECUTION_INTENT_CAPTURE_SECTION_SENTENCES,
...
function markdownFor(report) {
  const lines = [
    ...
    `Hash: ${report.contractDocPageReleaseExecutionIntentCaptureSectionRegressionHash}`,
    `Generated: ${report.generatedAt}`,
    '',
    '## Summary',
    '',
    `- Actual doc page release execution intent capture sections ok: ${report.summary.actualOk}`,
    ...
    `- Release execution intent capture sections: ${report.summary.sectionCount}/${report.summary.contractCount}`,
    ...REPORT_CONTRACT_DOC_PAGE_RELEASE_EXECUTION_INTENT_CAPTURE_SECTION_SENTENCES.map((binding) => countLine(report, binding)),
    ...
    '## Contracts',
    '',
    `| Contract | Docs | Section | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_EXECUTION_INTENT_CAPTURE_SECTION_SENTENCES.map((binding) => binding.label).join(' | ')} | Order |`,
    `| --- | --- | --- | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_EXECUTION_INTENT_CAPTURE_SECTION_SENTENCES.map(() => '---').join(' | ')} | --- |`,
    ...report.actual.contracts.map((contract) => `| ${contract.contractId} | ${contract.docsPath} | ${contract.sectionPresent} | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_EXECUTION_INTENT_CAPTURE_SECTION_SENTENCES.map((binding) => contractCell(contract, binding)).join(' | ')} | ${contract.orderValid} |`),
    '',
    '## Scenarios',
    '',
    ...
    '- Synthetic fixture proves missing release execution intent capture bindings, intent order drift, and shared-doc mapping drift fail closed.',
...
export function buildReportContractDocPageReleaseExecutionIntentCaptureSectionRegressionLatest({
  generatedAt = new Date().toISOString(),
} = {}) {
  return buildReportContractDocPageReleaseExecutionIntentCaptureSectionRegressionReport({
...
function main() {
  ...
  const report = buildReportContractDocPageReleaseExecutionIntentCaptureSectionRegressionLatest();
  const reportFiles = writeReports(report);
  process.stdout.write(`${JSON.stringify({
    ok: report.ok,
    status: report.status,
    contractDocPageReleaseExecutionIntentCaptureSectionRegressionHash: report.contractDocPageReleaseExecutionIntentCaptureSectionRegressionHash,
