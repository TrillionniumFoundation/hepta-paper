...
import {
  REPORT_CONTRACT_DOC_PAGE_RELEASE_POST_ACTION_BACKGROUND_RUNNER_DENIAL_SECTION_SENTENCES,
...
function markdownFor(report) {
  const lines = [
    ...
    `Hash: ${report.contractDocPageReleasePostActionBackgroundRunnerDenialSectionRegressionHash}`,
    `Generated: ${report.generatedAt}`,
    '',
    '## Summary',
    '',
    `- Actual doc page release post-action background runner denial sections ok: ${report.summary.actualOk}`,
    ...
    `- Release post-action background runner denial sections: ${report.summary.sectionCount}/${report.summary.contractCount}`,
    ...REPORT_CONTRACT_DOC_PAGE_RELEASE_POST_ACTION_BACKGROUND_RUNNER_DENIAL_SECTION_SENTENCES.map((binding) => countLine(report, binding)),
    ...
    '## Contracts',
    '',
    `| Contract | Docs | Section | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_POST_ACTION_BACKGROUND_RUNNER_DENIAL_SECTION_SENTENCES.map((binding) => binding.label).join(' | ')} | Order |`,
    `| --- | --- | --- | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_POST_ACTION_BACKGROUND_RUNNER_DENIAL_SECTION_SENTENCES.map(() => '---').join(' | ')} | --- |`,
    ...report.actual.contracts.map((contract) => `| ${contract.contractId} | ${contract.docsPath} | ${contract.sectionPresent} | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_POST_ACTION_BACKGROUND_RUNNER_DENIAL_SECTION_SENTENCES.map((binding) => contractCell(contract, binding)).join(' | ')} | ${contract.orderValid} |`),
    '',
    '## Scenarios',
    '',
    ...
    '- Synthetic fixture proves missing release post-action background runner denial bindings, post-action queue consumption order drift, and shared-doc mapping drift fail closed.',
    '- Does not rewrite report files except this exported latest report.',
    '- No provider/model calls.',
    '- No browser automation, upload, submit, message, acceptance, payment, deployment, channel-state fetch, local state transition, queue consumption, background runner, runner dispatch, queue ack, or execution permission grant.',
    '',
  ];
  return `${lines.join('\n')}\n`;
}
...
export function buildReportContractDocPageReleasePostActionBackgroundRunnerDenialSectionRegressionLatest({
  generatedAt = new Date().toISOString(),
} = {}) {
  return buildReportContractDocPageReleasePostActionBackgroundRunnerDenialSectionRegressionReport({
...
function main() {
  ...
  const report = buildReportContractDocPageReleasePostActionBackgroundRunnerDenialSectionRegressionLatest();
  const reportFiles = writeReports(report);
  process.stdout.write(`${JSON.stringify({
    ok: report.ok,
    status: report.status,
    contractDocPageReleasePostActionBackgroundRunnerDenialSectionRegressionHash: report.contractDocPageReleasePostActionBackgroundRunnerDenialSectionRegressionHash,
