...
import {
  REPORT_CONTRACT_DOC_PAGE_RELEASE_POST_ACTION_PROVIDER_SPEND_DENIAL_SECTION_SENTENCES,
...
function markdownFor(report) {
  const lines = [
    ...
    `Hash: ${report.contractDocPageReleasePostActionProviderSpendDenialSectionRegressionHash}`,
    `Generated: ${report.generatedAt}`,
    '',
    '## Summary',
    '',
    `- Actual doc page release post-action provider spend denial sections ok: ${report.summary.actualOk}`,
    ...
    `- Release post-action provider spend denial sections: ${report.summary.sectionCount}/${report.summary.contractCount}`,
    ...REPORT_CONTRACT_DOC_PAGE_RELEASE_POST_ACTION_PROVIDER_SPEND_DENIAL_SECTION_SENTENCES.map((binding) => countLine(report, binding)),
    ...
    '## Contracts',
    '',
    `| Contract | Docs | Section | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_POST_ACTION_PROVIDER_SPEND_DENIAL_SECTION_SENTENCES.map((binding) => binding.label).join(' | ')} | Order |`,
    `| --- | --- | --- | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_POST_ACTION_PROVIDER_SPEND_DENIAL_SECTION_SENTENCES.map(() => '---').join(' | ')} | --- |`,
    ...report.actual.contracts.map((contract) => `| ${contract.contractId} | ${contract.docsPath} | ${contract.sectionPresent} | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_POST_ACTION_PROVIDER_SPEND_DENIAL_SECTION_SENTENCES.map((binding) => contractCell(contract, binding)).join(' | ')} | ${contract.orderValid} |`),
    '',
    '## Scenarios',
    '',
    ...
    '- Synthetic fixture proves missing release post-action provider spend denial bindings, post-action deployment order drift, and shared-doc mapping drift fail closed.',
...
export function buildReportContractDocPageReleasePostActionProviderSpendDenialSectionRegressionLatest({
  generatedAt = new Date().toISOString(),
} = {}) {
  return buildReportContractDocPageReleasePostActionProviderSpendDenialSectionRegressionReport({
...
function main() {
  ...
  const report = buildReportContractDocPageReleasePostActionProviderSpendDenialSectionRegressionLatest();
  const reportFiles = writeReports(report);
  process.stdout.write(`${JSON.stringify({
    ok: report.ok,
    status: report.status,
    contractDocPageReleasePostActionProviderSpendDenialSectionRegressionHash: report.contractDocPageReleasePostActionProviderSpendDenialSectionRegressionHash,
