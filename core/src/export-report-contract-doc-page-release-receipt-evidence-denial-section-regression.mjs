...
import {
  REPORT_CONTRACT_DOC_PAGE_RELEASE_RECEIPT_EVIDENCE_DENIAL_SECTION_SENTENCES,
...
function markdownFor(report) {
  const lines = [
    ...
    `Hash: ${report.contractDocPageReleaseReceiptEvidenceDenialSectionRegressionHash}`,
    `Generated: ${report.generatedAt}`,
    '',
    '## Summary',
    '',
    `- Actual doc page release receipt-evidence denial sections ok: ${report.summary.actualOk}`,
    ...
    `- Release receipt-evidence denial sections: ${report.summary.sectionCount}/${report.summary.contractCount}`,
    ...REPORT_CONTRACT_DOC_PAGE_RELEASE_RECEIPT_EVIDENCE_DENIAL_SECTION_SENTENCES.map((binding) => countLine(report, binding)),
    ...
    '## Contracts',
    '',
    `| Contract | Docs | Section | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_RECEIPT_EVIDENCE_DENIAL_SECTION_SENTENCES.map((binding) => binding.label).join(' | ')} | Order |`,
    `| --- | --- | --- | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_RECEIPT_EVIDENCE_DENIAL_SECTION_SENTENCES.map(() => '---').join(' | ')} | --- |`,
    ...report.actual.contracts.map((contract) => `| ${contract.contractId} | ${contract.docsPath} | ${contract.sectionPresent} | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_RECEIPT_EVIDENCE_DENIAL_SECTION_SENTENCES.map((binding) => contractCell(contract, binding)).join(' | ')} | ${contract.orderValid} |`),
    '',
    '## Scenarios',
    '',
    ...
    '- Synthetic fixture proves missing release receipt-evidence denial bindings, receipt-evidence order drift, and shared-doc mapping drift fail closed.',
...
export function buildReportContractDocPageReleaseReceiptEvidenceDenialSectionRegressionLatest({
  generatedAt = new Date().toISOString(),
} = {}) {
  return buildReportContractDocPageReleaseReceiptEvidenceDenialSectionRegressionReport({
...
function main() {
  ...
  const report = buildReportContractDocPageReleaseReceiptEvidenceDenialSectionRegressionLatest();
  const reportFiles = writeReports(report);
  process.stdout.write(`${JSON.stringify({
    ok: report.ok,
    status: report.status,
    contractDocPageReleaseReceiptEvidenceDenialSectionRegressionHash: report.contractDocPageReleaseReceiptEvidenceDenialSectionRegressionHash,
