...
import {
  REPORT_CONTRACT_DOC_PAGE_RELEASE_RUNNER_EXECUTION_GATE_SECTION_SENTENCES,
...
function markdownFor(report) {
  const lines = [
    ...
    `Hash: ${report.contractDocPageReleaseRunnerExecutionGateSectionRegressionHash}`,
    `Generated: ${report.generatedAt}`,
    '',
    '## Summary',
    '',
    `- Actual doc page release runner execution gate sections ok: ${report.summary.actualOk}`,
    ...
    `- Release runner execution gate sections: ${report.summary.sectionCount}/${report.summary.contractCount}`,
    ...REPORT_CONTRACT_DOC_PAGE_RELEASE_RUNNER_EXECUTION_GATE_SECTION_SENTENCES.map((binding) => countLine(report, binding)),
    ...
    '## Contracts',
    '',
    `| Contract | Docs | Section | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_RUNNER_EXECUTION_GATE_SECTION_SENTENCES.map((binding) => binding.label).join(' | ')} | Order |`,
    `| --- | --- | --- | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_RUNNER_EXECUTION_GATE_SECTION_SENTENCES.map(() => '---').join(' | ')} | --- |`,
    ...report.actual.contracts.map((contract) => `| ${contract.contractId} | ${contract.docsPath} | ${contract.sectionPresent} | ${REPORT_CONTRACT_DOC_PAGE_RELEASE_RUNNER_EXECUTION_GATE_SECTION_SENTENCES.map((binding) => contractCell(contract, binding)).join(' | ')} | ${contract.orderValid} |`),
    '',
    '## Scenarios',
    '',
    ...
    '- Synthetic fixture proves missing release runner execution gate bindings, runner-execution-gate order drift, and shared-doc mapping drift fail closed.',
...
export function buildReportContractDocPageReleaseRunnerExecutionGateSectionRegressionLatest({
  generatedAt = new Date().toISOString(),
} = {}) {
  return buildReportContractDocPageReleaseRunnerExecutionGateSectionRegressionReport({
...
function main() {
  ...
  const report = buildReportContractDocPageReleaseRunnerExecutionGateSectionRegressionLatest();
  const reportFiles = writeReports(report);
  process.stdout.write(`${JSON.stringify({
    ok: report.ok,
    status: report.status,
    contractDocPageReleaseRunnerExecutionGateSectionRegressionHash: report.contractDocPageReleaseRunnerExecutionGateSectionRegressionHash,
