...
const defaultReportPath = path.join(packageRoot, 'reports', 'read-only-release-health-latest.json');
...
function main() {
  ...
  const validation = validateReadOnlyReleaseHealthManifest({
    manifest,
    actor: 'design-production-core.validate-readonly-release-health',
  });
  const blockers = [
    ...(readError ? [{ level: 'error', code: 'release_health_report_read_failed', notes: readError.message }] : []),
    ...validation.blockers,
  ];
  ...
  console.log(JSON.stringify({
    ok,
    status: ok ? validation.status : 'fail_readonly_release_health_validation',
