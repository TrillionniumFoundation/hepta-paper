...
current pure Node source increments in one identity-preserving path:
...
The pipeline exists to prevent callers from copying or inventing hashes between
stages. It is not a production control-plane service, execution dispatcher,
commit sequencer, module qualifier or authority transfer.
...
The candidate template deliberately has no `stateSnapshotHash` field. The
pipeline inserts the hash produced by the snapshot builder. The selection
template deliberately has no planning/snapshot/frontier/constraint/objective or
price hash fields. The pipeline inserts those values from the canonical
frontier.

Unknown repeated identity fields fail rather than being compared and silently
normalized. This removes an entire class of cross-stage splice bugs.

## 3. Shared clock and validity interval

One explicit integer clock observation is supplied to all three stages. It must
be inside the ECMAScript Date range. No stage reads ambient wall clock.

A planning request and a selection request may not outlive the snapshot from
which they were derived. Existing candidate, module-binding and frontier expiry
checks still apply. The resulting decision expires at the earliest snapshot,
frontier or selection deadline.
...
Malformed stage records, accessors, unknown fields, cross-stage stale candidates,
invalid module bindings, infeasible hard constraints and expired inputs retain
the typed errors of their owning stage. The pipeline does not catch a denial and
replace it with an empty frontier, fallback candidate, weaker snapshot or
successful decision.
...
- machine schemas and versioned compatibility for the pipeline result;
