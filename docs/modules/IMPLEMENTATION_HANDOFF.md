...
**Resident health and intake:** The [current intake V1 contract](../../rust/crates/hepta-paper-service/src/machine_intake/HANDOFF.md) describes actual configuration/static files, template budgets, provider precedence, generation-one SQLite/WAL state and the real current-intake health CLI. The [strict receipt contract](../../rust/crates/hepta-paper-service/src/strict_machine_intake_reconciliation/HANDOFF.md) adds original-format receipt/binding inspection to that actual intake observation. Neither diagnostic implements topic-producer V2 authority or publisher/lifecycle execution. The separately callable [external qualification V3 reader](../../rust/crates/hepta-paper-service/src/external_qualification_configuration/HANDOFF.md) supplies actual configuration/command/credential/public-trust observations and an original-format diagnostic inspection; full health now consumes it through resident prerequisites; it does not run external qualification or grant recovery authority. The separately callable [stored evidence readers](../../rust/crates/hepta-paper-service/src/qualification_stored_evidence/HANDOFF.md) consume the actual original SQLite pointer/state, committed WAL and byte-bound mirror with bounded schema/data parsing and source rechecks; they validate stored bindings only, close all descriptors before return and do not turn recorded inspection booleans into current authority. The existing runtime-image status reader now uses a [bounded private WAL snapshot](../../rust/crates/hepta-paper-service/src/runtime_image_reproducibility/PUBLICATION_READ_HANDOFF.md), with original effective-UID policy and actual signature/context checks; source SHM is not repaired. Its writer and online reconciliation paths retain their separate lifecycle contracts. The separately callable [recovery configuration inspector](../../rust/crates/hepta-paper-service/src/external_action_recovery_configuration/HANDOFF.md) derives original diagnostics from the actual private recovery file and V3 owner. Real synthetic signature fixtures reproduce the original release-trust/recovery-contract conflict; successful configuration identity is not capability verification. The separately callable [actual resident prerequisite inspector](../../rust/crates/hepta-paper-service/src/resident_prerequisites/HANDOFF.md) now sequences these producers with active-key signature, canonical time, code, state and cost/configuration bindings, the full original configuration readiness predicate and stable report hashes. Its source observations are completed and sequential; the [full health CLI composition](../../rust/crates/hepta-paper-service/src/SUPERVISOR_FULL_HEALTH_HANDOFF.md) now consumes them with actual passive state safety, while resident execution and independent recovery-purpose trust remain open. Full mode wins the exit criterion even when strict mode also produces a ready diagnostic. Native base health now reads effective committed WAL privately and does not recreate source SHM.
...
**Topic-producer source prerequisite:** The [native profile loader](../../rust/crates/hepta-paper-service/src/topic_producer_profile/HANDOFF.md) adds actual canonical builtin profile, dataset-content and original implementation-file observations with retained bounded owners. It is separately callable and must drop source descriptors before business SQLite. V2 intake remains blocked until actual producer status and V2 admission/genesis/rotation authority are connected; this source diagnostic does not claim producer execution or independent acceptance.
