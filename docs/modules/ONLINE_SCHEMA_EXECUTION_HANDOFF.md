...
The local full-scope planner and signed reservation are implemented. The live executor must consume these opaque objects and preserve their subject, source/root pins and fresh lease checks throughout every filesystem and SQLite write; passing a caller Boolean or a detached serialized receipt is insufficient.

The [journal normalization stage](SCHEMA_JOURNAL_NORMALIZATION_HANDOFF.md) now implements actual ten-database normalization, identity-bound stale-SHM handling, live signed-lease checks, durable per-instance progress and recovery across process death. Remaining execution work includes all-database exclusive installation locks, signed genesis/metadata installation and v1-to-v2 metadata replacement constraints, lease checks before each installation commit, committed-but-unrecorded installation recovery, final post-inventory/pristine validation and actual authority finalization/observation. External linearizability and deployment qualification remain separate from local implementation.
