...
Online externally fenced publication remains a separate integration boundary.
...
| `process.rs`, `process/identity.rs` | Bounded configuration, executable/interpreter/environment/credential identity, independent concurrent verifier processes |
...
Remaining work is explicit: connect the shared online mutation coordinator to
the receipt adapter and mirror side-effect permit lifecycle; obtain the actual R
