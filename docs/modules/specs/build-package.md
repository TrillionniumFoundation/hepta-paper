...
Manifest: [`../manifests/build-package.v1.json`](../manifests/build-package.v1.json)  
Registry: [`../../system/truth/modules.v1.json`](../../system/truth/modules.v1.json)

## Identity

```text
moduleId: module.build-package
...
Build reproducible manuscript and release-candidate packages from exact inputs, emitting content-addressed inventories, SBOM/provenance, and verification data.
...
- exact source tree and manuscript
- toolchain/image/lock identities
- build policy
- package inventory requirements

Outputs:

- PDF/package artifacts
- reproducibility manifest and SBOM
...
Use pinned non-root runtimes, read-only inputs, bounded generated-code execution, content-addressed outputs, and separate release signing authority.
...
Toolchain/image/lock/manuscript schema and package format are bound. Rebuild comparison declares byte-exact or semantic equivalence.
