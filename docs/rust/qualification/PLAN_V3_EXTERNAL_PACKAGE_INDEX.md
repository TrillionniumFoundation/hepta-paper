...
| `EXT-HOST-STORAGE-001` | `GAP-HOST-002` | WAL, SIGKILL, corruption, backup and restore harness | dedicated destructive-test-mount operator and independent observer | `external-host-storage-package-v1.schema.json` with content-hashed raw harness/review evidence | forbidden |
...
The aggregate verifier parses every package root itself. A valid outer signature and matching payload hash are necessary but insufficient: non-approved decisions, failed or inconclusive drills, open findings, duplicate role/authority identities and reviewer/operator collapse are rejected before durable acceptance.
