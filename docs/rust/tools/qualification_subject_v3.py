...
eligible workflow run attempt. A canonical successful run is selected per
workflow, but any non-canonical run mutation at or after the canonical update is
...
def parse_timestamp(value: Any) -> datetime:
    ...
    try:
        ...
        fail(f"workflow_run_timestamp_invalid:{value}:{error}")
    if parsed.tzinfo is None:
        fail(f"workflow_run_timestamp_not_aware:{value}")
    return parsed
...
def fetch_object(url: str, token: str) -> dict[str, Any]:
    value = fetch_json(url, token)
...
def required_policy(
    ...
    if (
        producers.get("schemaVersion") != 1
        or producers.get("kind") != "HeptaSourceCheckProducerManifestV1"
...
def pr_repository_identity(value: Any) -> tuple[int, str] | None:
    if not isinstance(value, dict):
        return None
...
def repository_matches(value: Any, expected_id: int, expected_full_name: str) -> bool:
    ...
    if isinstance(url, str):
        match = re.search(r"/repos/([^/]+/[^/?#]+?)/?$", urllib.parse.unquote(url))
        if match is not None:
            return match.group(1) == expected_full_name
    name = value.get("name")
    return isinstance(name, str) and name == expected_full_name.rsplit("/", 1)[-1]
    ...
    *,
    ...
    for row in rows:
        ...
        if (
            ...
            and repository_matches(base.get("repo"), base_repo_id, base_repo)
            and head.get("ref") == head_branch
            and head.get("sha") == head_commit
            and repository_matches(head.get("repo"), head_repo_id, head_repo)
        ):
            return True
    return False
...
def normalize_steps(raw_steps: Any) -> list[dict[str, Any]]:
    ...
    if not isinstance(raw_steps, list):
        fail("job_step_collection_invalid")
...
def validate_history_freshness(
    ...
    if len(matches) != len(required_contexts) or {
        job["name"] for job in matches
    ...
    canonical_updated = parse_timestamp(canonical["updatedAt"])
    for run_id, latest in latest_by_run.items():
        if run_id == canonical["runId"]:
            continue
        if parse_timestamp(latest["updatedAt"]) >= canonical_updated:
            fail(
...
def assemble_subject(
    *,
    ...
    for history in histories:
        if history.get("historyHash") != sha256_value(history.get("eligibleRuns")):
            fail(f"producer_history_hash_invalid:{history.get('workflowPath')}")
    all_attempts = [
        attempt
        for history in histories
    ...
    watermark_attempt = max(all_attempts, key=lambda row: parse_timestamp(row["updatedAt"]))
    watermark = watermark_attempt["updatedAt"]
    selected = [
        {
    ...
    definition_subject = {
        ...
        "producers": sorted(
            ...
            key=lambda row: (row["workflowId"], row["workflowPath"]),
        ),
    }
    body = {
        "schemaVersion": 3,
...
def collect(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if (
        ...
        or source.get("requiredChecksSha256") != required_checks_sha256
        or source.get("producerManifestSha256") != producer_manifest_sha256
    ):
    ...
    base_repo_identity = pr_repository_identity(base.get("repo"))
    head_repo_identity = pr_repository_identity(head.get("repo"))
    ...
    if base_repo != args.repository or head_repo != args.repository:
        fail("cross_repository_qualification_subject_forbidden")
    ...
    parents = [
        row.get("sha")
        for row in merge_git.get("parents", [])
    ...
    subject = assemble_subject(
        repository={"id": base_repo_id, "fullName": args.repository},
        pull_request={
