...
class RSourceRouteTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.route = ROUTE.read_json(ROUTE.DEFAULT_ROUTE)
        cls.schema = ROUTE.read_json(ROUTE.DEFAULT_SCHEMA)

    def test_real_repository_route_verifies_content_without_authority(self) -> None:
        result = ROUTE.verify_route(ROOT, self.route, self.schema)
        self.assertEqual(
            result["status"],
    ...
    def test_materializer_binding_drift_fails_before_capture(self) -> None:
        ...
        with mock.patch.object(ROUTE, "load_materializer", return_value=fake):
            with self.assertRaisesRegex(ValueError, "materializer_binding_drift"):
                ROUTE.verify_route(ROOT, self.route, self.schema)
        ...
        def changed(root: Path, *args: str, **kwargs):
            if args[:2] == ("ls-tree", "HEAD"):
                return subprocess.CompletedProcess(args, 0,
                    "160000 commit " + "0" * 40 + "\t" + self.route["targetPath"] + "\n", "")
            return original(root, *args, **kwargs)

        with mock.patch.object(ROUTE, "git", side_effect=changed), \
                mock.patch.object(ROUTE, "load_materializer") as materializer:
    ...
    def test_historical_subtree_drift_fails_before_loading_materializer(self) -> None:
        ...
        def changed(root: Path, *args: str, **kwargs):
            if args[:2] == ("rev-parse", "18b20af983e32575a2faab7dd8fa721a61d2e68c:runtime-images/r-scientific/source-cas"):
    ...
    def test_cli_output_is_bounded_nonactivating_json(self) -> None:
        completed = subprocess.run(
            [sys.executable, str(TOOLS / "verify-r-source-route.py")],
            cwd=ROOT,
            text=True,
            capture_output=True,
            timeout=60,
