...
export function managedPrompt({
  ...
  return [
    ...
    'Return exactly one valid JSON object and no markdown. In addition to role-specific fields, include top-level status (exactly "completed" when blockers is empty, otherwise exactly "blocked"), summary (string), edits (array), checksRun (a flat array of strings only, never objects), and blockers (a flat array of strings only). A blocker means you could not produce the requested role output or permitted edits from the supplied snapshot. Do not report downstream-stage work, paths intentionally excluded by the mutation policy, or tools this transport never offered as blockers. If you produced every requested role field and every permitted edit, return status "completed" with an empty blockers array. Never return a status such as "completed_with_blockers". Never claim that tools or checks were executed; checksRun may list only static inspections performed from the supplied snapshot.',
