...
function validateMatrixEntry({ root, workspaceRoot, row, sourceEntry }) {
  const blockers = [];
  ...
  const testResults = behaviorTests.map((test) => {
    ...
    return { id: test?.id || null, path: testPath || null, ok: testBlockers.length === 0, blockers: testBlockers };
  });
  return {
    id: row?.id || null,
    sourcePath,
...
export function buildMigrationMatrixAudit({ root, entries }) {
