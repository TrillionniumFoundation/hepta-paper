...
export function inspectConfiguredDynamicFormalProjectClosureEngine({
  ...
  try {
    ...
    if (!blockers.length && activeProbeRequested) {
      ...
      const probe = sandboxTestDependencies === null
        ? executeDynamicFormalSandboxProbe(probeInput)
        : executeDynamicFormalSandboxProbeEngine(probeInput, {
          ...sandboxTestDependencies,
          sandboxProbeExecutor: spawnSyncImpl,
        });
...
export function inspectConfiguredDynamicFormalExecutionAuthorityEngine(
  options = {},
  sandboxTestDependencies = null,
) {
  const inspection = inspectConfiguredDynamicFormalProjectClosureEngine(
    options,
    sandboxTestDependencies,
  );
  let authority = null;
...
export function assertCurrentDynamicFormalExecutionAuthorityEngine(
  expected,
  options = {},
  sandboxTestDependencies = null,
) {
  ...
  const current = inspectConfiguredDynamicFormalExecutionAuthorityEngine(
    options,
    sandboxTestDependencies,
  );
