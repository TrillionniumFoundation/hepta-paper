...
import {
  createDynamicFormalSandboxProbeRunner,
} from './dynamic-formal-sandbox-probe-runner-factory.mjs';
import {
...
export function executeDynamicFormalSandboxProbeEngine({
  manifest,
  closure,
  projectRoot,
  projectScopeRoot,
  ...
  const sandboxProbeRunnerFactory = testDependencies?.sandboxProbeRunnerFactory
    || createDynamicFormalSandboxProbeRunner;
  const projectSnapshotRepository = testDependencies?.projectSnapshotRepository
    || createFormalProjectSnapshotRepository();
  const verifySandboxProbeReceipt = testDependencies?.verifySandboxProbeReceipt
    || verifyProductionOsSandboxWorkerReceipt;
  const blockers = [];
  ...
  try {
    ...
    if (snapshotBefore?.status !== 'formal_project_closure_verified') {
      ...
      if (testDependencies?.sandboxProbeExecutor) {
        runnerOptions.executor = testDependencies.sandboxProbeExecutor;
      }
      const runner = sandboxProbeRunnerFactory(runnerOptions);
