...
export function createSqliteDeliveryResponseOperations({
  ...
  return {
    recordResponse({ messageId, response, responseVerificationReceipt = null, leaseToken = null } = {}) {
      ...
      const lockedMessage = getApi().getOutbox(messageId);
      const existing = persistence.one(`SELECT * FROM submission_inbox WHERE response_id=${sqlText(response.responseId)} LIMIT 1;`);
      ...
      if (!lockedMessage || lockedMessage.dispatch_hash !== message.dispatch_hash
        ...
        getApi().quarantineInvalidIntake({ messageId, payload: response, failureCodes: ['response_state_or_lease_changed_before_commit'] });
      ...
      if (existing) {
        ...
        if (!identical) {
          persistence.rollback();
          getApi().quarantineInvalidIntake({ messageId, payload: response, failureCodes: ['duplicate_executor_response_conflict'] });
        ...
        if (existing.persisted_receipt_id !== preparedLedger.receiptId
          ...
          persistence.rollback();
          getApi().quarantineInvalidIntake({ messageId, payload: response, failureCodes: ['duplicate_response_receipt_mismatch'] });
