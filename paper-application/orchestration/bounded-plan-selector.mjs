...
function nullableIdentifier(value, code) {
  ...
  return text(value, code, 256, IDENTIFIER);
}
...
function captureResourceLimits(value) {
  const data = record(value, RESOURCE_FIELDS, RESOURCE_FIELDS, 'plan_resource_limits_invalid');
...
function captureEvaluation(value) {
  ...
  return Object.freeze({
    ...
    mutexGroup: nullableIdentifier(data.mutexGroup, 'plan_evaluation_mutex_invalid'),
  });
}
...
function captureRequest(value, frontier, nowEpochMs) {
  ...
  const evaluations = arrayValues(data.evaluations, 4096, 'plan_evaluations_invalid')
    .map(captureEvaluation)
...
function zeroResources() {
  return Object.freeze(Object.fromEntries(RESOURCE_FIELDS.map((field) => [field, 0])));
}
...
function resourcesFit(used, limits) {
  return RESOURCE_FIELDS.every((field) => used[field] <= limits[field]);
}
...
function maximumOpenBound(stack, incumbent) {
  let upper = incumbent?.utility ?? Number.MIN_SAFE_INTEGER;
  for (const node of stack) {
    if (node.bound > upper) upper = node.bound;
  }
  return upper;
}
...
export function selectBoundedGlobalPlan(input) {
  ...
  const candidates = frontier.candidates.map((candidate) => Object.freeze({
    ...
    if (left.evaluation.utilityMicrounits !== right.evaluation.utilityMicrounits) {
      return left.evaluation.utilityMicrounits > right.evaluation.utilityMicrounits ? -1 : 1;
    }
  ...
  while (stack.length && expandedNodes < request.expansionBudget) {
    ...
    if (node.index === candidates.length) {
      ...
      if (!incumbent || node.utility > incumbent.utility
        || (node.utility === incumbent.utility && lexicographicallyBefore(node.selected, incumbent.selected))) {
  ...
  if (exhausted) {
    ...
    upperBound = maximumOpenBound(stack, incumbent);
  }
