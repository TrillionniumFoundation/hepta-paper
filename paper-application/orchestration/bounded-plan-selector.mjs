...
function captureFrontier(value, nowEpochMs) {
  ...
  const expiresAt = timestamp(data.expiresAt, 'candidate_frontier_expiry_invalid');
...
function captureEvaluation(value) {
  ...
  return Object.freeze({
    ...
    utilityMicrounits: signedInteger(data.utilityMicrounits, 'plan_evaluation_utility_invalid'),
...
function captureRequest(value, frontier, nowEpochMs) {
  ...
  const deadline = timestamp(data.deadline, 'plan_selection_deadline_invalid');
