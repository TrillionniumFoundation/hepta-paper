...
const SNAPSHOT_PROJECTION_FIELDS = Object.freeze([
  ...
  'qualificationIdentity', 'sourceGeneration', 'observedAt', 'validUntil',
  'payload', 'payloadHash',
]);
const SNAPSHOT_FIELDS = Object.freeze([
  'schemaVersion', 'kind', 'status', 'snapshotId', 'moduleRegistryHash',
  'policySetHash', 'resourcePriceSnapshotHash', 'objectiveVersion',
  'qualifiedProjectionSetHash', 'issuedAt', 'builtAt', 'expiresAt',
  'projectionCount', 'projectionSetHash', 'projections',
...
export function captureQualifiedProjectionSourceSetV1(value) {
  const sources = dense(value, LIMITS.projections, 'snapshot_projection_source_set_invalid', 1)
    .map(normalizeSource)
    .sort((left, right) => {
      const l = `${left.projectionId}\0${left.moduleId}\0${left.moduleVersion}`;
      const r = `${right.projectionId}\0${right.moduleId}\0${right.moduleVersion}`;
...
function falseAuthority(value, code) {
  ...
  const data = exactRecord(value, fields, code);
  if (fields.some((field) => data[field] !== false)) fail(code);
...
function captureSnapshot(value) {
  ...
  const ordered = [...projections].sort((left, right) => left.projectionId.localeCompare(right.projectionId));
  ...
  const authority = falseAuthority(data.authority, 'control_plane_snapshot_authority_invalid');
  const body = Object.freeze({
    schemaVersion: 1,
    kind: 'ControlPlaneSnapshotV1',
    status: 'control_plane_snapshot_ready',
    ...
    moduleRegistryHash: hash(data.moduleRegistryHash, 'snapshot_registry_hash_invalid'),
    policySetHash: hash(data.policySetHash, 'snapshot_policy_hash_invalid'),
    resourcePriceSnapshotHash: hash(data.resourcePriceSnapshotHash, 'snapshot_resource_price_hash_invalid'),
    objectiveVersion: text(data.objectiveVersion, TOKEN, 'snapshot_objective_version_invalid'),
    qualifiedProjectionSetHash: hash(data.qualifiedProjectionSetHash, 'snapshot_projection_set_binding_invalid'),
    issuedAt: timestamp(data.issuedAt, 'snapshot_issued_at_invalid'),
    builtAt: timestamp(data.builtAt, 'snapshot_built_at_invalid'),
    expiresAt: timestamp(data.expiresAt, 'snapshot_expires_at_invalid'),
    projectionCount: projections.length,
    projectionSetHash,
    projections,
    consumerMustRevalidateBeforePlanning: true,
  ...
  if (Date.parse(body.issuedAt) > Date.parse(body.builtAt)
    || Date.parse(body.builtAt) > Date.parse(body.expiresAt)) {
    fail('control_plane_snapshot_time_order_invalid');
  }
...
export function revalidateControlPlaneSnapshotV1(value) {
  const input = exactRecord(value, [
    ...
    'resourcePriceSnapshotHash', 'currentProjectionGenerations',
