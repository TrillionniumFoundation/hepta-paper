...
function captureSnapshot(value) {
  ...
  for (const projection of projections) {
    const bytes = Buffer.byteLength(stableStringify(projection.payload), 'utf8');
    if (bytes > LIMITS.projectionBytes) fail('snapshot_projection_byte_limit');
    totalBytes += bytes;
    if (totalBytes > LIMITS.totalProjectionBytes) fail('snapshot_total_byte_limit');
  }
  const projectionSetHash = hashRecord('SnapshotProjectionSetV1', projections);
  ...
  const built = Date.parse(body.builtAt);
  const expires = Date.parse(body.expiresAt);
  if (Date.parse(body.issuedAt) > built || built > expires) {
    fail('control_plane_snapshot_time_order_invalid');
  }
  for (const projection of projections) {
    if (Date.parse(projection.observedAt) > built
      || built > Date.parse(projection.validUntil)
      || expires > Date.parse(projection.validUntil)) {
