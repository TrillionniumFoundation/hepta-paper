...
function nullableIdentifier(value, code) {
  return value === null ? null : text(value, code, 256, IDENTIFIER);
}
...
function nullableInteger(value, code, { nonnegative = false } = {}) {
  return value === null ? null : safeInteger(value,
    nonnegative ? 0 : Number.MIN_SAFE_INTEGER, Number.MAX_SAFE_INTEGER, code);
}
...
function falseAuthority(value, fields, code) {
...
function captureFrontier(value, nowEpochMs) {
  ...
  if ((candidateCount === 0) !== (data.status === 'empty')
    ...
    || (candidateCount === 1 && candidates[0].singletonReason === null)) {
  ...
  const body = Object.freeze({
    ...
    authority: falseAuthority(data.authority, FRONTIER_AUTHORITY,
...
function captureResourceVector(value, code) {
  ...
  return Object.freeze(Object.fromEntries(RESOURCE_FIELDS.map((field) => [
    field, safeInteger(data[field], 0, Number.MAX_SAFE_INTEGER, `${code}:${field}`),
  ])));
}
function captureEvaluation(value) {
  ...
  return Object.freeze({
    ...
    mutexGroup: nullableIdentifier(data.mutexGroup, 'verified_plan_evaluation_mutex_invalid'),
  });
}

function captureRequest(value, frontier, nowEpochMs) {
  const data = record(value, REQUEST_FIELDS, REQUEST_FIELDS, 'verified_plan_request_invalid');
...
function captureProof(value) {
  const data = record(value, PROOF_FIELDS, PROOF_FIELDS,
    'verified_plan_source_proof_invalid');
...
function captureSelection(value, request) {
  ...
  const body = Object.freeze({
    ...
    authority: falseAuthority(data.authority, SELECTION_AUTHORITY,
      'verified_plan_selection_authority_invalid'),
  });
  if (typeof body.frontierExhausted !== 'boolean' || body.expandedNodes > body.expansionBudget) {
  ...
  const feasibleShape = body.selectedCandidateIds !== null
    && body.selectedCandidatePayloadHashes !== null && body.resourceUsage !== null
    && body.objectiveLowerBoundMicrounits !== null
    && body.objectiveUpperBoundMicrounits !== null && body.optimalityGapMicrounits !== null;
  if (body.status === 'optimal') {
    const exactProof = {
      feasibleIncumbent: true,
      hardConstraintsSatisfied: true,
      infeasibilityProven: false,
      optimalSelectionProven: true,
      dominanceReductionAssumed: false,
    };
    if (!feasibleShape || !body.frontierExhausted
      || body.objectiveUpperBoundMicrounits !== body.objectiveLowerBoundMicrounits
      || body.optimalityGapMicrounits !== 0
      || stableStringify(body.proof) !== stableStringify(exactProof)) {
      throw failure('verified_plan_source_proof_inconsistent');
    }
  } else if (body.status === 'bounded_feasible') {
    if (!feasibleShape || body.frontierExhausted || !body.proof.feasibleIncumbent
      || !body.proof.hardConstraintsSatisfied || body.proof.infeasibilityProven
      || body.proof.optimalSelectionProven) {
      throw failure('verified_plan_source_proof_inconsistent');
    }
  } else {
    const emptyShape = body.selectedCandidateIds === null
      && body.selectedCandidatePayloadHashes === null && body.resourceUsage === null
      && body.objectiveLowerBoundMicrounits === null && body.optimalityGapMicrounits === null;
    if (!emptyShape || body.proof.feasibleIncumbent || body.proof.hardConstraintsSatisfied
      || body.proof.optimalSelectionProven
      || body.proof.infeasibilityProven !== (body.status === 'infeasible')
      || body.frontierExhausted !== (body.status === 'infeasible')
      || (body.status === 'infeasible') !== (body.objectiveUpperBoundMicrounits === null)) {
      throw failure('verified_plan_source_proof_inconsistent');
    }
  }
...
function candidateResources(candidate) {
  ...
  return Object.freeze({
    cpuMilliunits: scaled(candidate.resourceVector.cpuUnits, 'verified_plan_candidate_cpu_invalid'),
    gpuMilliunits: scaled(candidate.resourceVector.gpuUnits, 'verified_plan_candidate_gpu_invalid'),
...
function zeroResources() { return Object.fromEntries(RESOURCE_FIELDS.map((field) => [field, 0])); }
function evaluateSelection(ids, candidateById, evaluationById, request) {
  ...
  for (const id of ids) {
    ...
    utility = safeAdd(utility, evaluation.utilityMicrounits, 'verified_plan_objective_overflow');
...
function lexicographicallyBefore(left, right) {
  return right === null || compareText(left.join('\0'), right.join('\0')) < 0;
}
function exhaustiveOracle(candidates, candidateById, evaluationById, request) {
  ...
  for (let mask = 0; mask < combinations; mask += 1) {
    ...
    if (!best || evaluated.utility > best.utility
      || (evaluated.utility === best.utility && lexicographicallyBefore(ids, best.ids))) {
...
export function verifyFeasiblePlanSelection(input) {
  ...
  const exactEnumerationLimit = Object.hasOwn(root, 'exactEnumerationLimit')
    ? safeInteger(root.exactEnumerationLimit, 0, 20, 'verified_plan_exact_limit_invalid') : 18;
  ...
  for (let index = 0; index < selection.selectedCandidateIds.length; index += 1) {
    ...
    if (!candidate || candidate.candidatePayloadHash !== selection.selectedCandidatePayloadHashes[index]) {
  ...
  const sourceGap = safeAdd(selection.objectiveUpperBoundMicrounits,
    -feasible.utility, 'verified_plan_source_gap_overflow');
  if (sourceGap < 0 || selection.optimalityGapMicrounits !== sourceGap) {
