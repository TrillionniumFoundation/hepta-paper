...
const INPUT_FIELDS = Object.freeze([
  ...
  'maximumConcurrency', 'openTimeoutMs', 'componentTimeoutMs',
  'closeTimeoutMs', 'builderLimits',
]);
...
export async function collectPlanningStateSnapshot(input) {
  ...
  const openTimeoutMs = Object.hasOwn(root, 'openTimeoutMs')
    ? safeInteger(root.openTimeoutMs, 1, 600_000,
      'planning_snapshot_open_timeout_invalid') : 10_000;
  ...
  let snapshot = null;
  try {
    const openOperation = Promise.resolve().then(() => port.open(Object.freeze({
      snapshotRequest: request,
      snapshotRequestHash: requestHash,
      moduleBindings: Object.freeze(bindings),
      signal: controller.signal,
    })));
    session = captureSession(await raceOperation(openOperation, controller.signal,
      openTimeoutMs, 'planning_snapshot_session_open_timeout'), request);
