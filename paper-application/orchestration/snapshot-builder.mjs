...
function captureComponentBody(value, limits) {
  const data = record(value, COMPONENT_PAYLOAD_FIELDS, COMPONENT_PAYLOAD_FIELDS,
...
function captureComponent(value, limits) {
  const data = record(value, COMPONENT_FIELDS, COMPONENT_FIELDS, 'snapshot_component_invalid');
...
export function buildPlanningStateSnapshot(input) {
  ...
  for (const raw of rawComponents) {
    ...
    if (encodedBytes(component.payload) > requirement.maximumPayloadBytes) {
      throw failure('snapshot_component_payload_byte_limit_exceeded');
    }
    const componentBytes = encodedBytes(component);
    if (componentBytes > limits.maximumComponentBytes) {
