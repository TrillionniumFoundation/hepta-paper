...
export async function main({
  ...
  if (options.help) {
    stdout.write([
      ...
      '  --from-venue <name>       select only papers with matching venue provenance',
      '  --target <name>           bind the local campaign to a new target venue without mutating inventory',
      '  --write-queue             persist the venue migration manifest/review queue (also implicit with --execute)',
      '                            venue migration requires --mode local-review-loop --local-only',
      '  --skip-quality-gates     structural rewrite only; defer paper.json formal/empirical profiles to a later campaign',
