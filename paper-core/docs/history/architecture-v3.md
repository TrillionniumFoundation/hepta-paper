# Historical hepta-paper architecture v3
...
The vendored `core/` remains an accepted, hash-bound reference package. Paper
production hashing and workflow transitions use `workflow-kernel/`; the active
production reachable graph is forbidden from importing `core/src`. Its package
metadata, baseline and compatibility rules are documented in
`reference-and-compatibility-boundaries.md`.
...
The legacy `WorkflowStatePort` may materialize a hash-bound projection only
through the explicit `compat:legacy-workflow-projection` command. Production
batch execution does not compose or write it. It remains derived compatibility
state rather than an independent authority; its ledger receipt and hepta-native
records remain the audit basis, and a stored hash mismatch fails closed.

The campaign store is the sole operational DAG authority. Node leases carry an
attempt id, monotonic generation and revision; complete/fail/recovery updates
are compare-and-swap transitions. Executors persist an immutable prepared
result before integration, so recovery can integrate without repeating the
external side effect. Campaign state, node result, usage and event projection
commit together.

Automation hands a completed campaign to submission only through the typed
`AutomationPromotionCandidate` and immutable `CampaignReleaseBundle`. The
bundle binds campaign/attempt lineage, source snapshot and manifest, verified
package, promotion gate and immutable output hashes. Submission owns a
separate verification/consumption adapter and composition root.

Experimental OpenClaw TaskFlow may coordinate long-lived waits only through
its opt-in outer adapter; it is absent from production composition roots. It
may own a flow revision, current step, minimal hash-only
...
domain snapshot and recomputes native gates. See `experimental-taskflow-pilot.md`.
...
The portable CI gate verifies the accepted reference package, runtime harness, architecture
