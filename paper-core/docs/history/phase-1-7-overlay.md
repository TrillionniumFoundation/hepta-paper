...
- `submission` now includes approval packet, fresh venue evidence, replay guard,
  ...
  reconciliation.
- The historical `legacy-cleanup` mode audited old `paper_factory` code for
  adapter candidates and quarantined control-plane modules. It is retired; the
  current read-only replacement is `npm run migration:retirement-status`.
