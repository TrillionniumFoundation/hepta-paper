...
import { verifyOwnerAcceptanceDocument } from '../../migration/owner-acceptance.mjs';
import { capabilityTargetBindings, verifyCapabilityOperationalReceipt } from '../../migration/operational-proof-intake.mjs';
import { verifyAuthoritySignatures } from './authority-signatures.mjs';
...
export function verifyExternalIntake({ stagingRoot, workspaceRoot, runtimeRoot, releaseCommit, paperId = 'A_Theory_of__Expectations' } = {}) {
  ...
  const accepted = verifyOwnerAcceptanceDocument({
    ...
    trustStore: ownerTrustStore,
    familyManifest: matrix.ownerAcceptanceFamilyManifest,
  });
  ...
  const blockers = [
    ...
    ...ownerTrust.blockers,
    ...(accepted.size === matrix.entries.length ? [] : [`owner_acceptance_incomplete:${accepted.size}/${matrix.entries.length}`]),
  ...
  return Object.freeze({
    ...
    ownerRequired: matrix.entries.length,
