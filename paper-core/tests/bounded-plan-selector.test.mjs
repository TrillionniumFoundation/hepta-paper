...
function resourcesFor(specification) {
  return {
...
function bruteForce(problem) {
  ...
  for (let mask = 0; mask < (2 ** specifications.length); mask += 1) {
    const selected = specifications.filter((_, index) => (mask & (2 ** index)) !== 0);
    ...
    for (const item of selected) {
      ...
      const vector = resourcesFor(item);
...
test('mutex groups are hard constraints and require canonical identifiers', () => {
  const problem = buildProblem([
  ...
  assert.throws(() => solve(problem, {
    evaluations: problem.selectionRequest.evaluations.map((item, index) =>
      index === 0 ? { ...item, mutexGroup: 'not allowed space' } : item),
  }), { code: 'plan_evaluation_mutex_invalid' });
});
...
test('budget-limited no-incumbent is distinct from exhaustive infeasibility', () => {
  const feasible = buildProblem([{ id: 'a', utility: 5, cpuUnits: 1 }], {
    requiredCandidateIds: ['a'], resourceLimits: { cpuMilliunits: 1000 },
  });
  const bounded = solve(feasible, { expansionBudget: 1 });
  ...
  assert.equal(solve(feasible, { expansionBudget: 100 }).status, 'optimal');

  const impossible = buildProblem([{ id: 'a', utility: 5, cpuUnits: 2 }], {
    requiredCandidateIds: ['a'], resourceLimits: { cpuMilliunits: 1000 },
  });
  const complete = solve(impossible, { expansionBudget: 100 });
...
test('limited search with the empty feasible incumbent reports honest bounds', () => {
  const problem = buildProblem([{ id: 'a', utility: 10 }, { id: 'b', utility: 4 }]);
...
test('negative optional candidates are excluded in the optimal empty plan', () => {
  const result = solve(buildProblem([{ id: 'a', utility: -1 }, { id: 'b', utility: -2 }]));
  assert.equal(result.status, 'optimal');
...
test('equal-utility plans use deterministic lexical tie breaking', () => {
  const problem = buildProblem([{ id: 'b', utility: 5 }, { id: 'a', utility: 5 }], {
...
test('unknown required identities fail and impossible cardinality is proven infeasible', () => {
...
test('fractional CPU and GPU values require exact milliunit representation', () => {
...
test('output is immutable non-authorizing and binds selected payload identities', () => {
  ...
  assert.equal(result.selectedCandidatePayloadHashes[0],
    problem.frontier.candidates[0].candidatePayloadHash);
