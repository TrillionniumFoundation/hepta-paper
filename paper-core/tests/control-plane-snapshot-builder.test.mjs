...
function projection(projectionId, overrides = {}) {
  const source = sources.find((entry) => entry.projectionId === projectionId);
  return sealReadOnlyProjectionV1({
    schemaVersion: 1,
...
function resealSnapshot(snapshot, changes) {
  const { stateSnapshotHash: ignored, ...body } = snapshot;
  const changed = changes(structuredClone(body));
  return Object.freeze({
    ...changed,
    stateSnapshotHash: hashRecord('ControlPlaneSnapshotV1', changed),
  });
}

test('projection and source declaration order cannot change the snapshot', () => {
  const a = build();
  const reversedSources = [...sources].reverse();
  const b = buildControlPlaneSnapshotV1({
    request: request(),
    qualifiedProjectionSources: reversedSources,
    projections: [projection('module-registry'), projection('campaign-state')],
  });
  assert.deepEqual(a, b);
  assert.deepEqual(a.projections.map((entry) => entry.projectionId), [
    'campaign-state', 'module-registry',
  ]);
});

test('snapshot expiry is the minimum request/projection boundary', () => {
...
test('projection coverage is exact and duplicate identities are rejected', () => {
  ...
  const extra = sealReadOnlyProjectionV1({
    ...projection('campaign-state'),
    projectionId: 'extra',
...
test('qualified source substitution is detected by the build request', () => {
  ...
  assert.throws(() => build({ sources: changed }), {
...
test('currentness receipt accepts reordered exact generations', () => {
  const snapshot = build();
...
test('rehashing cannot extend expiry beyond a projection validity boundary', () => {
  ...
  const forged = resealSnapshot(snapshot, (body) => ({
    ...body,
    expiresAt: '2026-09-06T01:09:00.000Z',
  }));
...
test('rehashing cannot erase source age or qualification constraints', () => {
  const snapshot = build();
  const tooOld = resealSnapshot(snapshot, (body) => {
    body.projections[0].maximumAgeMilliseconds = 1;
    body.qualifiedProjectionSetHash = hashRecord('QualifiedProjectionSourceSetV1',
      body.projections.map((entry) => ({
        projectionId: entry.projectionId,
    ...
    body.buildRequestHash = hashRecord('SnapshotBuildRequestV1', {
      schemaVersion: 1, kind: 'SnapshotBuildRequestV1', snapshotId: body.snapshotId,
      moduleRegistryHash: body.moduleRegistryHash, policySetHash: body.policySetHash,
      ...
      issuedAt: body.issuedAt, builtAt: body.builtAt, deadline: body.deadline,
...
test('authority and payload mutations are rejected even with outer fields present', () => {
  const snapshot = build();
...
test('candidate routing binds the snapshot hash and rejects a changed generation', () => {
  const snapshot = build();
  ...
  const frontier = routeActionCandidatesV1({ planningRequest, qualifiedModules: planningModules,
    candidates: [candidate], observedAt: '2026-09-06T01:02:00Z' });
