...
const oneCpuUsage = Object.freeze({
  cpuMilliunits: 1000,
  gpuMilliunits: 0,
  memoryMiB: 0,
  storageBytes: 0,
...
test('large frontier is feasibility-verified but source optimality is not self-accepted', () => {
  ...
  assert.equal(receipt.proof.sourceUpperBoundVerified, false);
});
...
test('self-consistent resource-overcommitted selection is rejected', () => {
  ...
  const forged = reseal(value.selection, {
    ...
    resourceUsage: { ...oneCpuUsage, cpuMilliunits: 2000 },
...
test('recomputed bounded result with an upper bound below exact optimum is rejected', () => {
  const value = problem(undefined, { expansionBudget: 1 });
  const forged = reseal(value.selection, {
