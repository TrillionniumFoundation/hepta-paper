...
test('component and total byte limits are enforced', () => {
  const large = component(undefined, undefined, { payload: { text: 'x'.repeat(2048) } });
  ...
  const second = component('registry', 'module-registry', { revision: 9, payload: { text: 'y'.repeat(2048) } });
  assert.throws(() => buildPlanningStateSnapshot({
    request: request([
      requirement(undefined, undefined, { maximumPayloadBytes: 4096 }),
      requirement('registry', 'module-registry', { minimumRevision: 9, maximumPayloadBytes: 4096 }),
    ]),
    moduleBindings: [binding()],
    components: [large, second],
    nowEpochMs: NOW,
    limits: { maximumTotalBytes: 1024, maximumComponentBytes: 4096 },
