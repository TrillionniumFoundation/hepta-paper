...
} from '../../paper-composition/automation/personal-gpu-operational-gate-composition.mjs';
import {
...
export async function runPersonalGpuOperationalGateCli({
  ...
  let args;
  try {
    args = parseStrictCliArguments(argv, {
  ...
  const receiptPath = path.resolve(
    args.receipt || path.join(runtimeRoot, 'gpu-personal', 'personal-gpu-operational-receipt.json'),
  );
  try {
    if (args.check) {
      const existing = loadExistingReceipt(receiptPath);
      stdout.write(`${JSON.stringify(existing, null, 2)}\n`);
      return existing.personalProductionReady ? 0 : 2;
    }
    ...
    return result.receipt.personalProductionReady ? 0 : 2;
  } catch (error) {
