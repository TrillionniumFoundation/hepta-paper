...
fn exchange(
    ...
    exchange_using(fixture, generation, dispatcher, |client, _peers| {
        write_request_frame(client, request, BrokerFramePolicyV1::default()).unwrap();
        if lose_reply {
            None
        } else {
            Some(
                read_response_frame(client, BrokerResponseFramePolicyV1::default())
                    .unwrap()
                    .0,
            )
        }
...
fn exchange_using<T>(
    ...
    generation: u64,
    dispatcher: Arc<PreparedDispatcher>,
    transact: impl FnOnce(&mut UnixStream, &PeerPolicyV1) -> T,
) -> T {
    ...
    client
        ...
        .unwrap();
    let result = transact(&mut client, &peers);
    drop(client);
    handle
        .join()
...
#[test]
fn typed_client_does_not_turn_a_bad_signature_into_dispatch() {
    let fixture = Arc::new(TempTree::new());
    let dispatcher = dispatcher(fixture.clone(), false);
    let mut request = request(&fixture);
    request.request_capability.signature_base64 = Base64UrlUnpadded::encode_string(&[0; 64]);
    let denied = exchange_using(&fixture, 1, dispatcher.clone(), |client, peers| {
        hepta_codex_broker::dispatch_signed_operation(client, peers, &request, 30_000)
    });
    assert!(matches!(
        denied,
    ...
    assert_eq!(fixture.open_journal().operation_count().unwrap(), 0);
}
