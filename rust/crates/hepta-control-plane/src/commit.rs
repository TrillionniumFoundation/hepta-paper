...
impl CommitRequestV1 {
    /// Creates an internal typed commit command from an already verified result.
    pub fn new(
        plan_hash: Sha256Digest,
        sequence: u64,
        verified: VerifiedPreparedResultV1,
    ) -> Result<Self, ControlPlaneError> {
        if sequence == 0 || verified.result().plan_hash != plan_hash {
    ...
    /// Returns the plan-local sequence.
    #[must_use]
    ...
    /// Decomposes the command for an external sequencer implementation.
    #[must_use]
    pub fn into_parts(self) -> (Sha256Digest, u64, VerifiedPreparedResultV1) {
        (self.plan_hash, self.sequence, self.verified)
    }
}
...
pub struct FixtureCommitSequencerV1 {
    ...
    next_sequence_by_plan: BTreeMap<Sha256Digest, u64>,
...
impl FixtureCommitSequencerV1 {
    ...
    pub fn new(initial_state_hash: Sha256Digest) -> Self {
        Self {
            ...
            initial_state_hash,
            next_sequence_by_plan: BTreeMap::new(),
...
impl CommitSequencerV1 for FixtureCommitSequencerV1 {
    fn commit(&mut self, request: CommitRequestV1) -> Result<CommitReceiptV1, ControlPlaneError> {
        ...
        if let Some(receipt) = self.receipts_by_result.get(&request.verified.result_hash) {
            if receipt.plan_hash == request.plan_hash && receipt.sequence == request.sequence {
        ...
        let next_sequence = self
            .next_sequence_by_plan
            .get(&request.plan_hash)
            .copied()
            .unwrap_or(1);
        ...
        self.next_sequence_by_plan
            .insert(receipt.plan_hash.clone(), following_sequence);
