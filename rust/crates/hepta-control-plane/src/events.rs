...
impl BoundedEventLogV1 {
    ...
    /// Verifies a conservative future event/cardinality reservation.
    pub fn ensure_capacity(
        &self,
        additional_events: usize,
        maximum_new_subjects: usize,
        ...
        let subject_total = self
            .subjects
            .len()
            .checked_add(maximum_new_subjects)
            .ok_or(ControlPlaneError::ObservabilityBudgetExceeded)?;
        if event_total > self.maximum_events || subject_total > self.maximum_unique_subjects {
            return Err(ControlPlaneError::ObservabilityBudgetExceeded);
        }
        Ok(())
    }

    /// Appends one event atomically if both cardinality budgets remain satisfied.
    pub fn push(&mut self, event: ControlPlaneEventV1) -> Result<(), ControlPlaneError> {
        self.push_batch(vec![event])
    }

    /// Appends a complete batch atomically or leaves the log unchanged.
    pub fn push_batch(
        &mut self,
        mut events: Vec<ControlPlaneEventV1>,
    ) -> Result<(), ControlPlaneError> {
        if events.is_empty() {
            return Ok(());
        }
        ...
        for event in &events {
            subjects.insert(event.subject_hash.clone());
        }
        let event_total = self
            .events
            .len()
            ...
            .ok_or(ControlPlaneError::ObservabilityBudgetExceeded)?;
        if event_total > self.maximum_events || subjects.len() > self.maximum_unique_subjects {
            return Err(ControlPlaneError::ObservabilityBudgetExceeded);
        }
        let start = u64::try_from(self.events.len())
            .ok()
            .and_then(|value| value.checked_add(1))
            .ok_or(ControlPlaneError::ObservabilityBudgetExceeded)?;
        for (offset, event) in events.iter_mut().enumerate() {
            let offset = u64::try_from(offset)
                .map_err(|_| ControlPlaneError::ObservabilityBudgetExceeded)?;
            event.ordinal = start
                .checked_add(offset)
                .ok_or(ControlPlaneError::ObservabilityBudgetExceeded)?;
        }
