...
impl BoundedEventLogV1 {
    ...
    /// Verifies that a complete future event batch can fit before any side effect begins.
    pub fn ensure_capacity(
        &self,
        additional_events: usize,
        maximum_new_subjects: usize,
    ) -> Result<(), ControlPlaneError> {
        if maximum_new_subjects > additional_events {
            return Err(ControlPlaneError::ObservabilityBudgetExceeded);
        }
        ...
        let subject_count = self
            ...
            .ok_or(ControlPlaneError::ObservabilityBudgetExceeded)?;
        if event_count > self.maximum_events || subject_count > self.maximum_unique_subjects {
            return Err(ControlPlaneError::ObservabilityBudgetExceeded);
        }
        Ok(())
    }
