...
use crate::{
    CalibrationObservationV1, CalibrationPolicyV1, CalibrationReportV1, ControlPlaneError,
    PerformanceAssessmentV1, PerformanceBudgetV1, PerformanceSampleV1, assess_calibration_v1,
    assess_performance_v1, canonical_hash_v1,
};

/// Maximum accepted encoded request size for the isolated qualification command.
...
impl PerformanceQualificationSubjectV1 {
    ...
    pub fn subject_hash(&self) -> Result<Sha256Digest, ControlPlaneError> {
...
pub fn qualify_performance_v1(
...
) -> Result<PerformanceQualificationReceiptV1, ControlPlaneError> {
    ...
    let calibration = match &request.calibration_policy {
        Some(policy) => Some(assess_calibration_v1(
            policy,
            request.calibration_observations.as_slice(),
        )?),
        None => None,
    };
