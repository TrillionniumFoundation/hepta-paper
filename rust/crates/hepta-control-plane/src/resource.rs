...
pub struct ResourceAccountingReportV1 {
    ...
    /// Latest authoritative monotonic admission observation.
...
mod tests {
    ...
    fn allocator_never_oversubscribes_global_or_tenant_capacity() {
        ...
        allocator
            .reserve(request("a", "tenant-a", 60, 0), 0)
            .expect("first reservation");
        assert_eq!(
            allocator.reserve(request("b", "tenant-b", 50, 0), 0),
    ...
    #[test]
    fn failed_reconciliation_and_release_leave_accounting_unchanged() {
        let mut allocator = allocator();
        let reservation = allocator
            .reserve(request("a", "tenant-a", 60, 0), 0)
            .expect("reservation");
        let before = allocator.report().expect("before report");

        assert_eq!(
            allocator.reconcile(&reservation.reservation_id, capacity(61)),
            Err(ControlPlaneError::ReconciliationInvalid)
        );
        assert_eq!(allocator.report().expect("after reconcile"), before);
        assert_eq!(
            allocator.release("missing-reservation"),
            Err(ControlPlaneError::ReservationInvalid)
        );
        assert_eq!(allocator.report().expect("after release"), before);
    }
    ...
    fn weighted_drf_and_aging_eventually_prioritize_an_old_waiter() {
        let mut allocator = allocator();
        allocator
            .reserve(request("active-a", "tenant-a", 50, 0), 0)
