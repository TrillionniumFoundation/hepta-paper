...
| `command-surface ROOT [--write-package]` | Inspect or synchronize the local `package.json` command-script registry without invoking Node; `--write-package` performs a byte-compatible deterministic rewrite with Node's insertion order and JavaScript truthiness. The standalone Node classify/help/matrix flags remain outside this mapped route. |
