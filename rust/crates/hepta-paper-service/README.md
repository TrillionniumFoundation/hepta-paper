...
| `store-integrity IMMUTABLE_DB` | Emit the complete read-only Node logical-integrity report, including byte pre/post hashes, SQLite checks, receipt-ledger validation and fail-closed status. |
...
| `command-surface ROOT [--write-package\|--check-package\|--npm-aliases\|--ci-matrix]` | Inspect or synchronize the local `package.json` command-script registry without invoking Node; write/check preserve Node's insertion order and JavaScript truthiness, while aliases and CI matrix are deterministic read-only projections. The standalone classify/help modes remain outside this mapped route. |
