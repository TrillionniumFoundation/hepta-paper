...
use hepta_paper_service::workflow::{
    LocalWorkflowV1, WorkflowActionV1, initialize_local_workflow_v1, operate_local_workflow_v1,
};
use nix::fcntl::OFlag;
...
fn execute() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() < 3 {
        return Err("usage: init DEFINITION | status STATE HASH | advance STATE HASH THROUGH NOW | pause|resume|cancel STATE HASH REVISION NOW".into());
    }
