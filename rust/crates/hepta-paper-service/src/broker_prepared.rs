//! Read-only ingestion of an existing broker result into the ordinary service.
//! No provider dispatch, private key, second result store, or production grant.
...
fn validate_binding(
    ...
    if input.version != 1
        ...
        || execution.candidate.resources.provider_calls != 0
...
pub(crate) fn consume(
    ...
    let delivery = query_prepared_result(&stream, &policy, &captured.request, source.timeout_ms)
        .map_err(|_| ServiceError::Execution)?;
    captured.revalidate(source)?;
    let named = fs::symlink_metadata(&source.socket_path).map_err(|_| ServiceError::Filesystem)?;
    ...
    let evidence = serde_json::json!({
        ...
        "providerExecutionRequestedByConsumer": false,
