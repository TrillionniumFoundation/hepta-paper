...
/// JavaScript's parameterless `Array.prototype.sort()` compares UTF-16 code
/// units, unlike the locale-aware comparator used by `node_order` above.
...
pub fn evaluate_campaign_policy_v1(
  ...
  CampaignPolicyRequestV1::Descendants{nodes,root_node_id}=>{if root_node_id.is_empty(){return Err(CampaignPolicyError::Invalid)}let mut set=std::collections::BTreeSet::from([root_node_id]);loop{let mut changed=false;for n in &nodes{if !set.contains(&n.node_id)&&n.dependencies.iter().any(|d|set.contains(d)){set.insert(n.node_id.clone());changed=true}}if !changed{break}}let mut ids=set.into_iter().collect::<Vec<_>>();ids.sort_by(|left,right|js_default_compare(left,right));json!(ids)},
  CampaignPolicyRequestV1::FutureRound{nodes,after_round}=>{let mut ids=nodes.into_iter().filter(|n|n.round_index>after_round&&!CONVERGENCE.contains(&n.kind.as_str())&&n.status=="queued").map(|n|n.node_id).filter(|id|!id.is_empty()).collect::<Vec<_>>();ids.sort_by(|left,right|js_default_compare(left,right));ids.dedup();json!(ids)},
