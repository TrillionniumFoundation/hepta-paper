...
CREATE UNIQUE INDEX IF NOT EXISTS authority_live_global_sequence
ON authority_mutation(global_sequence) WHERE status != 'aborted';
CREATE TABLE IF NOT EXISTS authority_native_identity(
  singleton INTEGER PRIMARY KEY CHECK(singleton=1),
