...
impl Fixture {
    pub fn new() -> Self {
        ...
        let payload = objects
            .put(
                &serde_json::to_vec(&NativeJobV1::BrokerPrepared {
                    input: input.clone(),
                })
                .unwrap(),
            )
        ...
        registry
            .register(ModuleManifestV1 {
                ...
                execution: ModuleExecutionV1::InProcess {
                    implementation_hash: broker_prepared_implementation_hash_v1(&source).unwrap(),
                },
            })
            .unwrap();
        ...
        let request = CodexExecutionRequestV1 {
            ...
            absolute_deadline_unix_ms: 100_000,
