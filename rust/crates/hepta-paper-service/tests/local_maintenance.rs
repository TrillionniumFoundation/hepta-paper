...
use hepta_paper_service::{
    ObjectStoreV1,
    maintenance::{LocalMaintenanceSessionV1, verify_local_backup_v1},
};
...
impl Fixture {
    ...
    fn object_path(&self) -> PathBuf {
        self.state
            .join("objects")
            .join(self.object.as_str().trim_start_matches("sha256:"))
    }
    fn backup(&self) -> (PathBuf, Sha256Digest) {
        let bundle = self.parent.join("backup");
        let receipt = LocalMaintenanceSessionV1::acquire(&self.state)
            .expect("lease")
            .backup(&bundle)
            .expect("backup");
...
fn actual_sqlite_and_cas_bytes_round_trip_without_recovery_promotion() {
    ...
    assert!(!receipt.semantic_recovery_verified);
    assert_eq!(
        fs::read(f.state.join("campaign.sqlite")).expect("source"),
        fs::read(bundle.join("payload/campaign.sqlite")).expect("copy")
    );
    assert_eq!(
        fs::read(f.object_path()).expect("object"),
        fs::read(
            bundle
                .join("payload/objects")
                .join(f.object.as_str().trim_start_matches("sha256:"))
        )
        .expect("object copy")
    );
}
...
fn corrupt_cas_is_rejected_without_repair() {
    ...
    assert!(
        LocalMaintenanceSessionV1::acquire(&f.state)
            .expect("lease")
            .inspect()
            .is_err()
    );
...
fn unknown_private_files_are_not_archived() {
    let f = Fixture::new();
    private_write(
        &f.state.join("credentials.json"),
        b"fixture secret must not be copied",
    );
...
fn existing_destination_is_not_adopted_or_overwritten() {
    ...
    assert_eq!(
        fs::read(destination.join("preserve")).expect("preserved"),
        b"unchanged"
    );
}
