...
fn autonomous_research_cli_uses_durable_workflow_and_recovers_across_processes() {
    ...
    let definition_file = temp.0.join("autonomous-research-definition.json");
    fs::write(&definition_file, serde_json::to_vec(&definition).unwrap()).unwrap();
    fs::set_permissions(&definition_file, fs::Permissions::from_mode(0o600)).unwrap();

    let prepared = Command::new(env!("CARGO_BIN_EXE_hepta-paper-rust"))
        .args([
            "autonomous-research",
            "--action",
            "prepare",
    ...
    let hash = prepared["definitionHash"].as_str().unwrap().to_owned();

    let launched = Command::new(env!("CARGO_BIN_EXE_hepta-paper-rust"))
        .args([
            "autonomous-research",
            "--action",
            "launch",
    ...
    assert_eq!(launched["operationSucceeded"], true);
    assert_eq!(launched["committedSteps"], 2);
    assert_eq!(launched["workflowExecuted"], true);
    assert_eq!(launched["externalActionPerformed"], false);

    // A fresh process must reconstruct progress from the durable SQLite/CAS owner.
    let observed = Command::new(env!("CARGO_BIN_EXE_hepta-paper-rust"))
        .args([
            "autonomous-research",
            "--action",
            "status",
    ...
    assert_eq!(observed["committedSteps"], 2);
    let revision = observed["campaignRevision"].as_u64().unwrap().to_string();

    let cancelled = Command::new(env!("CARGO_BIN_EXE_hepta-paper-rust"))
        .args([
            "autonomous-research",
            "--action",
            "cancel",
    ...
    assert_eq!(cancelled["operationSucceeded"], true);
    assert_eq!(cancelled["campaignState"], "cancelled");

    let terminal = Command::new(env!("CARGO_BIN_EXE_hepta-paper-rust"))
        .args([
            "autonomous-research",
            "--action",
            "status",
    ...
    assert_eq!(terminal["campaignState"], "cancelled");
    assert_eq!(terminal["committedSteps"], 2);
}
