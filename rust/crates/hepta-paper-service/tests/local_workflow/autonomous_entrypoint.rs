...
fn request(temp: &Temp, mut def: LocalWorkflowV1) -> PathBuf {
    ...
    def.template.writer_lease.expires_at_unix_ms = now + 120_000;
    def.validate().unwrap();
...
fn invoke(path: &Path, action: &str, extra: &[&str]) -> std::process::Output {
    Command::new(env!("CARGO_BIN_EXE_hepta-paper-rust"))
        .args(["autonomous-research", "--campaign-id", "campaign-service", "--workflow-file"])
        .arg(path)
        .args(["--action", action])
        .args(extra)
        .output()
...
fn autonomous_prepare_launch_replay_and_converge_use_the_existing_owner() {
    ...
    assert!(!temp.state().exists(), "prepare must not initialize or adopt state");
    ...
    let old_cli = Command::new(env!("CARGO_BIN_EXE_hepta-local-workflow"))
        .arg("status").arg(temp.state()).arg(hash).output().unwrap();
    assert!(old_cli.status.success());
    let old_progress: Value = serde_json::from_slice(&old_cli.stdout).unwrap();
    assert_eq!(done["workflow"], old_progress, "both commands must read the same SQLite/CAS owner");
    let count = attempt_count(&temp);
    assert_eq!(success(invoke(&path, "converge", &[]))["workflow"], old_progress);
    assert_eq!(attempt_count(&temp), count);
}
...
fn autonomous_pause_resume_cancel_are_revision_bound_and_terminal() {
    ...
    let revision = first["workflow"]["campaignRevision"].as_u64().unwrap().to_string();
    assert!(!invoke(&path, "pause", &["--expected-revision", "9999"]).status.success());
    ...
    let revision = paused["workflow"]["campaignRevision"].as_u64().unwrap().to_string();
    let resumed = success(invoke(&path, "resume", &["--expected-revision", &revision]));
    let revision = resumed["workflow"]["campaignRevision"].as_u64().unwrap().to_string();
    let cancelled = success(invoke(&path, "cancel", &["--expected-revision", &revision]));
    let revision = cancelled["workflow"]["campaignRevision"].as_u64().unwrap().to_string();
    assert!(!invoke(&path, "resume", &["--expected-revision", &revision]).status.success());
    assert!(!invoke(&path, "launch", &[]).status.success());
    assert_eq!(count, attempt_count(&temp));
    let hash = cancelled["definitionHash"].as_str().unwrap().parse().unwrap();
    assert_eq!(status(&temp, &hash).campaign_state, CampaignStateV1::Cancelled);
}
...
fn autonomous_rejects_changed_definition_and_foreign_campaign_without_dispatch() {
    ...
    assert!(!invoke(&path, "status", &["--paper-id", "another-paper"]).status.success());
    assert_eq!(count, attempt_count(&temp));
...
fn autonomous_local_authority_and_request_bounds_fail_before_state_creation() {
    let temp = Temp::new();
    let path = request(&temp, definition(&temp));
    for extra in [vec!["--launch-mode", "production-run"], vec!["--launch-mode", "golden-bootstrap"], vec!["--require-full-ready"], vec!["--through-steps", "0"], vec!["--through-steps", "129"], vec!["--expected-revision", "0"], vec!["--workflow-file", "/another/request"]] {
