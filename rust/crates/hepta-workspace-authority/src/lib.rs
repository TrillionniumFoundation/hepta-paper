...
use bound::{
    BoundEntry, BoundKind, descriptor_path, open_bound_entry, same_identity,
    same_stable_object, verify_path_binding,
};
...
impl WorkspaceRootV1 {
    ...
    pub fn open(path: &Path, expected_uid: Option<u32>) -> Result<Self, WorkspaceError> {
        ...
        Ok(Self {
            ...
            link_count: opened.metadata.nlink(),
...
pub fn materialize_attempt_v1(
    ...
    copy_tree(&source.descriptor, source.device, &destination.file)?;
    destination.file.sync_all()?;
    parent.descriptor.sync_all()?;
    source.verify_identity()?;
    parent.verify_identity()?;
    verify_path_binding(&public_destination, &destination.metadata)?;
...
fn copy_tree(
    ...
    for entry in entries {
        ...
        match opened.kind {
            BoundKind::Directory => {
                ...
                copy_tree(&opened.file, source_device, &target.file)?;
    ...
    if !same_stable_object(&source_before, &source_after) {
        return Err(WorkspaceError::EntryChanged);
    }
...
fn copy_bound_file_with_hook(
    ...
    if !same_stable_object(&source.metadata, &source_before) {
        return Err(WorkspaceError::EntryChanged);
    }
    let mut output = OpenOptions::new()
        ...
        .open(destination)?;
    let output_identity = output.metadata()?;
    let result = (|| {
        let (copied_hash, copied_bytes) = copy_and_hash(&mut source.file, &mut output)?;
        output.sync_all()?;
        let after_copy = source.file.metadata()?;
        if !same_stable_object(&source_before, &after_copy)
            || copied_bytes != source_before.size()
        {
            return Err(WorkspaceError::EntryChanged);
        }
        ...
        if !same_stable_object(&source_before, &after_verify)
    ...
    if let Err(error) = result {
        drop(output);
        if verify_path_binding(destination, &output_identity).is_ok() {
...
fn walk_inventory(
    ...
    if !same_stable_object(&directory_before, &directory_after) {
        return Err(WorkspaceError::EntryChanged);
    }
    Ok(())
}
...
fn hash_bound_file_with_hook(
    ...
    if !same_stable_object(&entry.metadata, &before) {
        return Err(WorkspaceError::EntryChanged);
    }
    let (first_hash, first_bytes) = hash_open_file(&mut entry.file)?;
    let after_first = entry.file.metadata()?;
    if !same_stable_object(&before, &after_first) || first_bytes != before.size() {
        return Err(WorkspaceError::EntryChanged);
    }
    between_passes();
    ...
    if !same_stable_object(&before, &after_second)
