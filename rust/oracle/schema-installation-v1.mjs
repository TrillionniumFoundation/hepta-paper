...
const {installAutonomousResearchOnlineSchemaTransitionLocks}=await import('../../paper-adapters/automation/autonomous-research-online-schema-transition-installation.mjs');
function installationPreview(input) {
 assertRoot(input.root);const authority=authorities.get(input.root);const reservation=authority.lastReceipt;
 if(!reservation||reservation.signature!==input.reservation.signature||autonomousResearchOnlineMutationSignedPayload(reservation)!==autonomousResearchOnlineMutationSignedPayload(input.reservation))throw new Error('original_signed_receipt_mismatch');
 const root=fs.mkdtempSync('/tmp/hepta-schema-source-rust-install-preview-');const locks=[];
 try {
  for(const row of input.plan.instances) {
   const source=path.join(authority.runtimeRoot,row.sourceRelativePath);if(fileSha256HashSync(source)!==row.expectedNormalizedSourceSha256)throw new Error('signed_normalized_preimage_mismatch');
   const candidate=path.join(root,locks.length+'.image');fs.copyFileSync(source,candidate,fs.constants.COPYFILE_EXCL);fs.chmodSync(candidate,0o600);
   const database=new DatabaseSync(candidate);database.exec('PRAGMA busy_timeout=0; BEGIN EXCLUSIVE;');
   locks.push({database,instance:{role:row.databaseRole,instanceId:row.databaseInstanceId},planInstance:row,lockedSchemaHash:schema.schemaTransitionExactSchemaHash(database)});
  }
  return installAutonomousResearchOnlineSchemaTransitionLocks({locks,plan:input.plan,reservation,clock:{now:()=>new Date(NOW)},commitSafetyMarginMs:1000,state:{phase:'reserved',installations:[]},statePath:path.join(root,'progress.json')});
