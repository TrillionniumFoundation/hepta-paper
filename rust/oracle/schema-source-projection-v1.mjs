...
const {withAutonomousResearchStateDatabasePrivateSnapshot}=await import('../../paper-adapters/automation/autonomous-research-state-database-inventory.mjs');
...
for await(const line of rl){try{const input=JSON.parse(line);const value=input.operation==='fixture'?fixture(input):input.operation==='normalize-copy'?normalizeCopy(input):input.operation==='full-fixture'?fullFixture(input):input.operation==='full-plan'?fullPlan(input):input.operation==='reserve-maintenance'?reserveMaintenance(input):projection(input);process.stdout.write(JSON.stringify({ok:true,value,profile:productionOracleProfile()})+'\n');}catch(error){process.stdout.write(JSON.stringify({ok:false,error:error.message,profile:productionOracleProfile()})+'\n');}}
