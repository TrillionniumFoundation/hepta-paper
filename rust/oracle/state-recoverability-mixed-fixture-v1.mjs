...
function backupClient(f,mode='valid'){
 ...
 // The original restore validator compares JSON member order. The synthetic
 // authority emits the same fixed head projection as the original broker.
 if(journal)journal.databaseHeads=journal.databaseHeads.map(({databaseRole,databaseInstanceId,sequence,hash,schemaHash,stateHash})=>({databaseRole,databaseInstanceId,sequence,hash,schemaHash,stateHash}));
 ...
 const invoke=q=>{if(mode==='timeout')throw Error('autonomous_research_state_backup_authority_timeout');const expires=new Date(Date.parse(q.requestedAt)+60000).toISOString();let value;
  ...
  if(journal&&q.kind==='AutonomousResearchStateBackupAuthorityCurrentHeadRequest'){value.headSequence=journal.entries.length;value.headHash=journal.globalHash;}
